...
You MUST use this exact code snippet for consistency, any other method WILL cause an error due to the format of the data:
<reaction_smiles_extraction>
rsmi = node["metadata"]["rsmi"]
reactants = rsmi.split(">")[0].split(".")
product = rsmi.split(">")[-1]
</reaction_smiles_extraction>
This is essential as the reaction smiles is in reactants>reagents>products format, and the reagents are not needed for this analysis.
...
Ideally generate 2-4 strategies per route to give a good description of the synthetic approach.
...
To ensure the generated code can be correctly parsed, you MUST follow these strict rules:
Place All Imports at the Top: All import statements MUST be at the very top of the <code> block, outside of and before any function definitions. Do not place imports inside any function.
One Top-Level Function Per Strategy: Each strategy must be contained within a single top-level Python function. Helper functions like dfs_traverse MUST be defined inside this main function.
Clean Code Block: The <code> block must contain ONLY valid Python code (and comments). Do not add any conversational text.
If you are checking for the transformation of functional groups, ensure that your code reliably checks that the correct FG is actually changed in the reaction, by concurrently checking for the relevant reaction, or through the use of atom-mapping in the reaction SMILES to track Functional groups and rings between reactants and products.
Correct vs. Incorrect Structure Example:
INCORRECT (Imports are inside the function):

def my_strategy(route):
    import re  # <-- WRONG LOCATION
    from rdkit import Chem # <-- WRONG LOCATION

    def dfs_traverse(node):
        ...
    return True
CORRECT (Imports are at the top-level):

import re
from rdkit import Chem

def my_strategy(route):
    # This is the single top-level function

    def dfs_traverse(node): # Helper is correctly nested
        ...
    
    dfs_traverse(route)
    return True
You MUST follow the CORRECT structure.
...
You MUST wrap the code in <code> ... </code> tags.
</code>
