...
def main(route):
    ...
    def is_tetrahydroisoquinoline(smiles):
        ...
        if checker.check_ring("piperidine", smiles) and checker.check_ring(
            "benzene", smiles
        ):
            # Get ring indices
            ...
            if piperidine_indices and benzene_indices:
                ...
                piperidine_atoms = set(
                    [atom for ring in piperidine_indices for atom in ring]
                )
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            ...
            if is_tetrahydroisoquinoline(node["smiles"]):
                ...
                print(
                    f"Found tetrahydroisoquinoline scaffold at depth {depth}: {node['smiles']}"
                )
            ...
            try:
                ...
                if product_has_thiq and not reactants_have_thiq:
                    ...
                    reaction_type = (
                        "Pictet-Spengler" if is_pictet_spengler else "forming"
                    )
                    print(
