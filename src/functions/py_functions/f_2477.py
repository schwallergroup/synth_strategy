        ...
        if node["type"] == "reaction" and depth <= 1:  # Final or penultimate step
            try:
                ...
                if checker.check_reaction("Boc amine deprotection", rsmi):
                    print(
                        f"Found Boc amine deprotection reaction at depth {depth}: {rsmi}"
                    )
                ...
                for reaction_type in deprotection_reactions:
                    if checker.check_reaction(reaction_type, rsmi):
                        print(
                            f"Found {reaction_type} reaction at depth {depth}: {rsmi}"
                        )
                        has_late_deprotection = True
                        return
                ...
                if any(
                    ...
                    print(
                        f"Found Boc group removal at depth {depth} (manual check): {rsmi}"
                    )
                    has_late_deprotection = True
                    return
                ...
                if any(
                    ...
                    print(
                        f"Found silyl group removal at depth {depth} (manual check): {rsmi}"
                    )
                    has_late_deprotection = True
                    return
