...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if (
            ...
            trifluoroacetamide_pattern = Chem.MolFromSmarts(
                "[N]-C(=O)-C([F])([F])([F])"
            )
    ...
    late_stage_sulfonamide = (
        sulfonamide_formation_depth > trifluoroacetamide_deprotection_depth
    )
    ...
    print(
        f"Sequential amine protection-deprotection with sulfonamide formation: {result}"
    )
    return result
