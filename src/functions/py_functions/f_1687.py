...
def main(route):
    ...
    def is_common_reagent(smiles):
        ...
        if smiles in common_reagents or any(
            reagent in smiles for reagent in common_reagents
        ):
            return True
        ...
        if node["type"] == "reaction" and node.get("metadata", {}).get("rsmi"):
            ...
            if atom_based_count is not None:
                if atom_based_count > 1:
                    is_linear = False
                    print(
                        f"Detected convergent step based on atom mapping in reaction: {rsmi}"
                    )
