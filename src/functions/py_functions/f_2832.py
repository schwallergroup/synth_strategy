...
def main(route):
        ...
        if node["type"] == "mol" and depth == 0:  # Final product
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                reactants_with_nitrile = [
                    r for r in reactants if checker.check_fg("Nitrile", r)
                ]

                # Get nitrile atom indices in product and reactants
                product_nitrile_indices = checker.get_fg_atom_indices(
                    "Nitrile", product
                )
                ...
                # If both product and some reactants have nitriles, check if new nitriles were introduced
                    ...
                    reactants_nitrile_count = sum(
                        len(checker.get_fg_atom_indices("Nitrile", r))
                        for r in reactants
                    )
                ...
                if product_has_nitrile and any(
                    ...
                    for rxn_type in cyanation_reactions
                ):
                ...
                if product_has_nitrile:
                    ...
                    if any(
                        any(checker.check_fg(fg, r) for fg in halide_fgs)
                        for r in reactants
                    ):
                        print(
                            f"Potential cyanation via halide substitution at depth {depth}"
                        )
                        if cyanation_at_depth is None or depth < cyanation_at_depth:
                            cyanation_at_depth = depth
                ...
                if product_has_nitrile:
                    ...
                    if any(
                        any(checker.check_fg(fg, r) for fg in carbonyl_fgs)
                        for r in reactants
                    ):
                        print(
                            f"Potential cyanation via carbonyl transformation at depth {depth}"
                        )
                        if cyanation_at_depth is None or depth < cyanation_at_depth:
                            cyanation_at_depth = depth
    ...
    if is_late_stage:
        ...
        if cyanation_at_depth is not None:
            print(
                f"Cyanation detected but not at late stage (depth {cyanation_at_depth})"
            )
