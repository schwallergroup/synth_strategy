...
def main(route):
        ...
        if node["type"] == "reaction":
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if product and all(r for r in reactants):
                    ...
                    has_methanol = any(
                        r.HasSubstructMatch(methanol_pattern) for r in reactants
                    )
                    ...
                    mesyl_chloride_pattern = Chem.MolFromSmarts(
                        "[Cl]-[#16](=[#8])(=[#8])-[#6]"
                    )
                    ...
                    has_sec_amine = any(
                        r.HasSubstructMatch(sec_amine_pattern) for r in reactants
                    )
                    has_mesylate = any(
                        r.HasSubstructMatch(mesylate_pattern) for r in reactants
                    )
                    has_tert_amine_product = product.HasSubstructMatch(
                        tert_amine_pattern
                    )
    ...
    strategy_present = (
        found_esterification
        and found_ester_reduction
        and found_mesylation
