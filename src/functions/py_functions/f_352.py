...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        if node["type"] == "reaction":
            ...
            if num_reactants > 1:
                ...
                print(
                    f"Detected convergent step at depth {depth} with {num_reactants} reactants"
                )
            else:
    ...
    linear_steps_after_convergence = sum(
        1 for d in linear_steps if d > earliest_convergent_depth
    )
