...
def main(route):
    ...
    if "children" in final_product and len(final_product["children"]) > 0:
        ...
        try:
            ...
            is_cooh_deprotection = checker.check_reaction(
                "COOH ethyl deprotection", rsmi
            )
            ...
            if (
                ...
                if (
                    reactants_have_ester
                    and product_has_acid
                    and has_hydrolysis_reagents
                ):
                    print(
                        "Late-stage ester hydrolysis detected through manual pattern check!"
                    )
                    final_step_is_hydrolysis = True
