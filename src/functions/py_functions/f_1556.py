...
def main(route):
        ...
        if node["type"] == "reaction":
                ...
                if is_heteroaromatic_with_halide(reactant):
                    ...
                    # Fallback check for SNAr-like transformations
                        ...
                        if reactant_mol and product_mol:
                            print(
                                f"Found potential SNAr reaction (halide displacement): {rsmi}"
                            )
                            snar_reactions.append((rsmi, depth, product))
                            break
            ...
            if product_mol and checker.check_fg("Acylhydrazine", product):
                ...
                if not all(checker.check_fg("Acylhydrazine", r) for r in reactants):
                    ...
                    if (
                        not hydrazide_formation_reaction
                        or depth < hydrazide_formation_reaction[1]
                    ):
                ...
                for reactant in next_reactants:
                    ...
                    for ring in heteroaromatic_rings:
                        if checker.check_ring(
                            ring, current_product
                        ) and checker.check_ring(ring, reactant):
                            print(
    ...
    are_sequential = (
        are_reactions_sequential(snar_reactions) if has_multiple_snar else False
    )
    ...
    if has_late_stage_hydrazide and not has_multiple_snar:
        ...
        if len(snar_reactions) >= 1:
            print(
                "Fallback: Considering single SNAr reaction with hydrazide formation as a match."
            )
