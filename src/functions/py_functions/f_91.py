...
def main(route):
    ...
    if has_ester_to_acid and has_acid_to_amide:
        ester_to_acid_depth = next(
            t[1] for t in transformations if t[0] == "ester_to_acid"
        )
        acid_to_amide_depth = next(
            t[1] for t in transformations if t[0] == "acid_to_amide"
        )
        correct_sequence = ester_to_acid_depth > acid_to_amide_depth

    print(
        f"Transformations: {transformations}, Has correct sequence: {correct_sequence}"
    )
