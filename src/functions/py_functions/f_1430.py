...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if depth == 0 and node["type"] == "mol":
            final_product_has_morpholine = checker.check_ring(
                "morpholine", node["smiles"]
            )
        ...
        if node["type"] == "reaction":
            if "rsmi" in node.get("metadata", {}):
                ...
                # Check for epoxide presence (as a fallback)
                    ...
                    checker.check_fg("Oxirane", r) for r in reactants
                ) or checker.check_fg("Oxirane", product):
                    if epoxide_stage is None or depth < epoxide_stage:
                        epoxide_stage = depth
                ...
                if (
                    ...
                    if (
                        aminoethanol_coupling_stage is None
                        or depth < aminoethanol_coupling_stage
                    ):
                        aminoethanol_coupling_stage = depth
                        print(
                            f"Found aminoethanol coupling at depth {depth}, rsmi: {rsmi}"
                        )
                ...
                if checker.check_ring("morpholine", product) and not any(
                    ...
                    if (
                        morpholine_formation_stage is None
                        or depth < morpholine_formation_stage
                    ):
                        morpholine_formation_stage = depth
                        print(
                            f"Found morpholine formation at depth {depth}, rsmi: {rsmi}"
                        )

                # Check for morpholine ring opening with epoxide
                if checker.check_reaction("Ring opening of epoxide with amine", rsmi):
                    if (
                        morpholine_opening_stage is None
                        or depth < morpholine_opening_stage
                    ):
                ...
                # Alternative check for morpholine ring opening
                    ...
                    if (
                        ...
                        morpholine_opening_stage = depth
                        print(
                            f"Found morpholine ring opening at depth {depth}, rsmi: {rsmi}"
                        )
