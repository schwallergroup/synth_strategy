            ...
            if depth <= 2:
                ...
                for reaction_type in nucleophilic_sub_reactions:
                    if checker.check_reaction(reaction_type, rsmi):
                        print(
                            f"Found late-stage nucleophilic substitution: {reaction_type}"
                        )
                ...
                has_leaving_group = any(
                    any(checker.check_fg(lg, r) for lg in leaving_groups)
                    for r in reactants
                )
                has_nucleophile = any(
                    any(checker.check_fg(nuc, r) for nuc in nucleophiles)
                    for r in reactants
                )
                ...
                if not late_stage_substitution:
                    ...
                    if has_chloro and has_amine:
                        ...
                        if "NH" in product and not any(
                            "NH" in r and "Cl" in r for r in reactants
                        ):
                            print(
