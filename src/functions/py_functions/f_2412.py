...
def main(route):
        ...
        # For reaction nodes, check if heterocycles are formed
            ...
            try:
                ...
                if is_heterocycle_forming:
                    ...
                    print(
                        f"Recorded heterocycle formation at depth {depth}: {formed_heterocycles}"
                    )
    ...
    if not heterocycle_formations and molecules_with_heterocycles:
        print(
            "No direct heterocycle formations detected, inferring from molecule data..."
        )

        # Sort molecules by depth
        sorted_molecules = sorted(
            molecules_with_heterocycles.values(), key=lambda x: x["depth"]
        )
        ...
        if len(sorted_molecules) >= 2:
            ...
            if len(set(depths)) >= 2:
                print(
                    f"Found molecules with heterocycles at different depths: {depths}"
                )
    ...
    if len(heterocycle_formations) >= 2:
        ...
        for i, formation in enumerate(heterocycle_formations):
            print(
                f"  {i+1}. Depth {formation['depth']}: {formation['formed_heterocycles']}"
            )
