    ...
    def is_cf3_attached_to_heterocycle(mol_smiles):
        ...
        for heterocycle in heterocycle_types:
            if checker.check_ring(heterocycle, mol_smiles):
                ...
                try:
                    cf3_indices = checker.get_fg_atom_indices(
                    ...
                    for cf3_group in cf3_indices:
                        ...
                        if isinstance(cf3_group, tuple) and len(cf3_group) > 0:
                            if (
                                isinstance(cf3_group[0], tuple)
                                and len(cf3_group[0]) > 0
                            ):
                    ...
                    print(
                        f"Error checking CF3-heterocycle attachment for {heterocycle}: {e}"
                    )
                    print(
                        f"CF3 indices structure: {type(cf3_indices)}, content: {cf3_indices}"
                    )
                    if cf3_indices:
                        print(f"First CF3 group type: {type(cf3_indices[0])}")
                    print(
                        f"Ring indices structure: {type(ring_indices)}, content: {ring_indices}"
                    )
                    if ring_indices:
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "reaction":
            try:
                ...
                product_has_cf3_heterocycle = is_cf3_attached_to_heterocycle(
                    product_smiles
                )
                ...
                if product_has_cf3_heterocycle:
                    ...
                    if not reactants_with_cf3:
                        print(
                            f"Detected trifluoromethyl heterocycle formation at depth {depth}"
                        )
                        ...
                        print(
                            f"CF3-heterocycle already present in reactants: {reactants_with_cf3}"
                        )
            except Exception as e:
