...
def main(route):
        ...
        if node["type"] == "reaction":
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                if checker.check_reaction("Reduction of nitro groups to amines", rsmi):
                    print(
                        f"Nitro reduction detected at depth {depth} using reaction checker"
                    )
                    ...
                    for reactant in reactants:
                        if checker.check_fg("Nitro group", reactant):
                            ...
                            if product_mol and checker.check_fg(
                                ...
                                print(
                                    f"Nitro reduction detected at depth {depth} using FG checker"
                                )
                                nitro_reduction_found = True
                                nitro_reduction_depth = depth
                                reduced_nitro_mol = product
    ...
    if nitro_reduction_found and nitro_reduction_depth is not None:
        print(
            f"Checking for cyclization after nitro reduction at depth {nitro_reduction_depth}"
        )
            ...
            if d < nitro_reduction_depth:  # Lower depth means later in synthesis
                ...
                if not subsequent_cyclization_found:
                    ...
                    try:
                        ...
                        product_rings = (
                            len(Chem.GetSSSR(product_mol)) if product_mol else 0
                        )
                        ...
                        if product_rings > reactants_rings:
                            ...
                            for reactant in rxn_reactants:
                                if reduced_nitro_mol and checker.check_fg(
                                    ...
                                    print(
                                        f"Amine-containing reactant found in cyclization"
                                    )
                                    subsequent_cyclization_found = True
                                    break
