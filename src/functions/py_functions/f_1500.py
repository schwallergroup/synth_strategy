        ...
        if node["type"] == "reaction" and depth <= 1:  # Late stage (depth 0 or 1)
            if "rsmi" in node.get("metadata", {}):
                ...
                if len(reactants) >= 3:
                    ...
                    has_isocyanide = any(
                        checker.check_fg("Isocyanide", r) for r in reactants
                    )

                    if is_ugi or is_a3_coupling or has_isocyanide:
                        late_stage_mcr = True
                        print(
                            f"Detected late-stage multi-component reaction at depth {depth}"
                        )
                        ...
                        if is_ugi:
                            ...
                            print(
                                "Contains isocyanide component (likely Ugi-type reaction)"
                            )
