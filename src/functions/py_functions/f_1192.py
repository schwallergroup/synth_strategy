...
def main(route):
        ...
        if node["type"] == "reaction":
            if "rsmi" in node.get("metadata", {}):
                ...
                if checker.check_reaction("Alcohol to triflate conversion", rsmi):
                    print(
                        f"Detected triflate formation reaction at depth {depth}: {rsmi}"
                    )
                ...
                if any(
                ...
                ) and not checker.check_fg("Triflate", product):
                    ...
                    if (
                        ...
                        or checker.check_reaction(
                            "Sonogashira acetylene_aryl OTf", rsmi
                        )
                        or checker.check_reaction(
                            "Sonogashira alkyne_alkenyl OTf", rsmi
                        )
                        or checker.check_reaction(
                            "Sonogashira acetylene_alkenyl OTf", rsmi
                        )
                    ...
                    # Check for nucleophilic substitution with amines
                        ...
                        print(
                            f"Detected triflate displacement by amine at depth {depth}: {rsmi}"
                        )
                        triflate_displacement_reactions.append((depth, rsmi))
                    # Check for other nucleophilic substitutions
                        ...
                        print(
                            f"Detected triflate displacement by alcohol at depth {depth}: {rsmi}"
                        )
                        triflate_displacement_reactions.append((depth, rsmi))
    ...
    def check_sequence():
        ...
        min_displacement_depth = min(
            [depth for depth, _ in triflate_displacement_reactions]
        )
    ...
    result = (
        has_triflate_formation or has_starting_triflate
    ) and has_triflate_displacement
    ...
    print(
        f"Starting triflate: {has_starting_triflate}, Sequence correct: {sequence_correct}"
    )
