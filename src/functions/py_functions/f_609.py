    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            ...
            if checker.check_reaction(
                ...
                if (
                    has_pyrazole_reactant
                    and has_pyrazole_product
                    and has_bromine_product
                ):
                    has_bromination = True
                    print(
                        f"Found pyrazole bromination reaction at depth {depth}: {rsmi}"
                    )
            ...
            if any(checker.check_reaction(rxn, rsmi) for rxn in n_alkylation_reactions):
                ...
                if has_pyrazole_reactant and has_pyrazole_product:
                    has_n_alkylation = True
                    print(
                        f"Found pyrazole N-alkylation reaction at depth {depth}: {rsmi}"
                    )
        ...
        if (
            ...
            if "Br" in node["smiles"] and checker.check_fg(
                "Aromatic halide", node["smiles"]
            ):
                has_bromination = True
                print(
                    f"Found brominated pyrazole molecule at depth {depth}: {node['smiles']}"
                )
