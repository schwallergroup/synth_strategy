        ...
        if node["type"] == "reaction" and depth == 1:
            try:
                ...
                if any("O=C1CCC(=O)N1Br" in r for r in reactants_smiles):
                    print(
                        "Detected N-bromosuccinimide (NBS) as a reactant, likely a bromination"
                    )
                    final_step_halogenation = True
                    return
                ...
                for reactant in reactant_mols:
                    for atom in reactant.GetAtoms():
                        symbol = atom.GetSymbol()
                        if symbol in halogen_symbols:
                            reactants_halogens[symbol] = (
                                reactants_halogens.get(symbol, 0) + 1
                            )
                ...
                if "Br" in rsmi:
                    ...
                    if "[Br:" in rsmi and "Br]" in product_smiles:
                        print(
                            "Detected bromine transfer in final step based on atom mapping"
                        )
                        final_step_halogenation = True
                        return
