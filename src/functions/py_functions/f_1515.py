...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
                ...
                if reactants_mol and product_mol:
                    ...
                    if product_aromatic_atoms > reactant_aromatic_atoms:
                        ...
                        is_oxidation_or_dehydrogenation = (
                            ...
                            or checker.check_reaction(
                                "Hydrogenation (triple to double)", rsmi
                            )
                        )
                        ...
                        has_cyclohexadiene = checker.check_ring(
                            "cyclohexane", reactants_smiles
                        )
                        ...
                        if (has_new_benzene and has_cyclohexadiene) or (
                            ...
                            print(
                                f"  CONFIRMED: Aromatization detected in forward direction"
                            )
                    ...
                    # Check for dearomatization (reverse direction - important for retrosynthetic traversal)
                        ...
                        has_benzene_in_reactants = checker.check_ring(
                            "benzene", reactants_smiles
                        )
                        has_benzene_in_products = checker.check_ring(
                            "benzene", product_smiles
                        )
                        ...
                        is_dearomatization_rxn = checker.check_reaction(
                            "Arene hydrogenation", rsmi
                        )

                        if is_dearomatization_rxn and has_benzene_in_reactants:
                            print(
                                f"  Arene hydrogenation detected with benzene in reactants"
                            )
                            print(
                                f"  CONFIRMED: Dearomatization detected (aromatization in retrosynthetic direction)"
                            )
                        ...
                        for rxn_type in aromatization_rxn_types:
                            if checker.check_reaction(rxn_type, rsmi):
                                ...
                                # For other reactions, check if benzene is involved
                                    ...
                                    print(
                                        f"  Benzene ring involved in {rxn_type} reaction"
                                    )
                                    aromatization_detected = True
                                    break
        ...
        for child in node.get("children", []):
            if (
                not aromatization_detected
            ):  # Stop traversal if we've already found aromatization
