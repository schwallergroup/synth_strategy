        ...
        if node["type"] == "reaction" and depth <= 1:
            ...
            try:
                ...
                if not all(reactants) or not product:
                    print(
                        f"Warning: Could not parse all molecules in reaction at depth {depth}"
                    )
                    return
                ...
                if has_nitro:
                    ...
                    reactant_rings = sum(
                        mol.GetRingInfo().NumRings() for mol in reactants
                    )
                    ...
                    if product_rings > reactant_rings:
                        print(
                            f"Ring count increased: {reactant_rings} -> {product_rings}"
                        )
                        ...
                        for ring_name in [
                            ...
                            if checker.check_ring(ring_name, product_smiles):
                                ...
                                print(
                                    f"Found heterocycle: {ring_name} in product at depth {depth}"
                                )
                                break
