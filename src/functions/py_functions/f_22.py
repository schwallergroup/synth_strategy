...
def main(route):
    ...
    if final_product_smiles is None:
        ...
        def find_lowest_depth_molecule(node, depth=0):
            ...
            if node["type"] == "mol":
                if (
                    final_product_smiles is None
                    or depth < find_lowest_depth_molecule.min_depth
                ):
                    final_product_smiles = node["smiles"]
        ...
        if node["type"] == "mol":
            ...
            if mol_smiles == final_product_smiles:
                if has_indole and has_thiazole:
                    ...
                    print(
                        f"Found both indole and thiazole in final product: {mol_smiles}"
                    )
                    print(
                        f"Indole present: {has_indole}, Thiazole present: {has_thiazole}"
                    )
    ...
    print(
        f"Both indole and thiazole in final product: {found_indole_thiazole_in_product}"
    )
