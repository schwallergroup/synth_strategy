...
def main(route):
    ...
    mol_to_reactions = (
        {}
    )  # Maps molecule SMILES to lists of [producing_rxn, consuming_rxn]
    reaction_count = 0
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            ...
            if smiles not in mol_to_reactions:
                mol_to_reactions[smiles] = [
                    None,
                    None,
                ]  # [producing_rxn, consuming_rxn]
            ...
            for child in node.get("children", []):
                if child["type"] == "reaction":
                    ...
                    if mol_to_reactions[smiles][0] is not None:
                        print(
                            f"Molecule {smiles} is produced by multiple reactions - not linear"
                        )
                        return False
    ...
    if intermediates != reaction_count - 1:
        print(
            f"Found {intermediates} intermediates but {reaction_count} reactions - not linear"
        )
        return False
