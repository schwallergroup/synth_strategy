...
def main(route):
    ...
    def is_oxadiazolone(mol_smiles):
        ...
        if checker.check_ring("oxadiazole", mol_smiles):
            if checker.check_fg("Ketone", mol_smiles) or checker.check_fg(
                "Amide", mol_smiles
            ):
        ...
        if node["type"] == "mol":
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                if is_oxadiazolone(product) and (
                    not oxadiazolone_info["detected"]
                    or depth < oxadiazolone_info["depth"]
                ):
                    oxadiazolone_info["detected"] = True
                    oxadiazolone_info["smiles"] = product
                    oxadiazolone_info["depth"] = depth
                    print(
                        f"Oxadiazolone ring detected in product at depth {depth}: {product}"
                    )
    ...
    # For debugging
    print(
        f"Hydrazide detected: {hydrazide_info['detected']} at depth {hydrazide_info['depth']}"
    )
    print(
