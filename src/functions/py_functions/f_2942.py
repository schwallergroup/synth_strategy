...
def main(route):
    ...
    def has_aromatic_chloride(mol_smiles):
        ...
        try:
            ...
            return (
                checker.check_fg("Aromatic halide", mol_smiles) and "Cl" in mol_smiles
            )
    ...
    def dfs_traverse(node, depth=0, parent_has_aryl_cl=None):
        ...
        if node["type"] == "mol":
            ...
            # For non-target molecules, check if aryl chloride is maintained
                ...
                if (
                    parent_has_aryl_cl
                    and not has_aryl_chloride
                    and not node.get("in_stock", False)
                ):
                    print(
                        f"Aryl chloride lost at depth {depth} in molecule: {mol_smiles}"
                    )
    ...
    if not target_has_aryl_chloride:
        print(
            "Target molecule doesn't have aryl chloride, so maintenance check is not applicable"
        )
        return False
