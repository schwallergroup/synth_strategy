...
def main(route):
                ...
                try:
                    ...
                    for rxn_type in ether_reactions:
                        if checker.check_reaction(rxn_type, rsmi):
                            print(
                                f"Ether formation detected: {rxn_type} at depth {depth}"
                            )
                    ...
                    if not ether_depths or ether_depths[-1] != depth:
                        ...
                        ether_in_product = len(
                            checker.get_fg_atom_indices("Ether", product)
                        )
    ...
    print(
        f"Heterocycle-first with late-stage ether strategy detected: {combined_strategy}"
    )
