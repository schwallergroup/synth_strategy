...
def main(route):
                        ...
                        try:
                            ...
                            for reactant in reactants:
                                ...
                                if reactant_mol is not None and product_mol is not None:
                                    ...
                                    cc_double_bond_pattern = Chem.MolFromSmarts(
                                        "[#6]=[#6]"
                                    )
                                    reactant_cc_bonds = len(
                                        reactant_mol.GetSubstructMatches(
                                            cc_double_bond_pattern
                                        )
                                    )
                                ...
                                for reactant in reactants:
                                    ...
                                    for fg in n_containing_fgs:
                                        if checker.check_fg(
                                            ...
                                            cn_disconnection_depth = depth
                                            print(
                                                f"Detected {fg} formation at depth {depth}"
                                            )
                                            break
    ...
    correct_sequence = (
        has_olefination
        and has_cn_disconnection
        and cn_disconnection_depth <= olefination_depth
    )

    print(f"Olefination found: {has_olefination} at depth {olefination_depth}")
    print(
        f"C-N disconnection found: {has_cn_disconnection} at depth {cn_disconnection_depth}"
    )
    print(
        f"Correct sequence (C-N after or with olefination in synthesis): {correct_sequence}"
    )
