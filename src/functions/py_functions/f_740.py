...
def main(route):
            ...
            try:
                ...
                if not (depth in oxidation_steps or depth in reduction_steps):
                    ...
                    if alcohol_in_product and (
                        ...
                        or ketone_in_reactants
                        or carboxylic_in_reactants
                    ):
                        reduction_steps.append(depth)
                        print(
                            f"Detected reduction (FG change) at depth {depth}: {rsmi}"
                        )
                    ...
                    carboxylic_in_product = checker.check_fg(
                        "Carboxylic acid", product_smiles
                    )

                    if alcohol_in_reactants and (
                        aldehyde_in_product
                        or ketone_in_product
                        or carboxylic_in_product
                    ):
                        oxidation_steps.append(depth)
                        print(
                            f"Detected oxidation (FG change) at depth {depth}: {rsmi}"
                        )
    ...
    for ox1 in oxidation_steps:
        for red in reduction_steps:
            if (
                red > ox1
            ):  # Reduction occurs earlier in synthesis (higher depth in retrosynthesis)
                for ox2 in oxidation_steps:
                    if (
                        ox2 > red
                    ):  # Second oxidation occurs even earlier (even higher depth)
                        print(
        ...
        for red in reduction_steps:
            if red < ox1:  # Reduction occurs later in synthesis (lower depth)
                for ox2 in oxidation_steps:
                    if (
                        ox2 < red
                    ):  # Second oxidation occurs even later (even lower depth)
                        print(
