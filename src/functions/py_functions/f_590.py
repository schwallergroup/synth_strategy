...
def main(route):
    ...
    def dfs_traverse(node, depth=0, path_has_boc=False):
        ...
        if node["type"] == "mol":
            ...
            if has_boc:
                print(
                    f"Boc group detected at depth {depth}, SMILES: {mol_smiles[:30]}..."
                )
                path_has_boc = True
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                is_boc_deprotection = checker.check_reaction(
                    "Boc amine deprotection", rxn_smiles
                )
    ...
    if not final_product_has_boc:
        ...
        boc_present_in_synthesis = any(
            path_has_boc for _, _, path_has_boc in current_path
        )
