...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol" and "smiles" in node:
            ...
            quinolone_pattern = Chem.MolFromSmarts(
                "c1nccc2ccccc12C(=O)"
            )  # Quinoline with carbonyl
            ...
            has_isoquinolone = (
                mol.HasSubstructMatch(isoquinolone_pattern) if mol else False
            )

            if not (
                has_quinoline or has_isoquinoline or has_quinolone or has_isoquinolone
            ):
