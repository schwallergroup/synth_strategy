...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol" and len(node.get("children", [])) > 1:
            print(
                f"Branching detected at depth {depth} - molecule has multiple reaction paths"
            )
            is_linear = False
            return
        ...
        if node["type"] == "reaction":
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if not is_linear_reaction_type:
                    ...
                    boronic_count = sum(
                        1 for r in reactants if is_boronic_derivative(r)
                    )
                ...
                if not is_linear_reaction_type:
                    ...
                    significant_reactants = [
                        r for r in reactants if not is_common_reagent(r)
                    ]
                ...
                print(
                    f"Warning: Reaction node at depth {depth} missing metadata or rsmi"
                )
