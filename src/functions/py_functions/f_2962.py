...
def main(route):
    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "mol":
            ...
            if any(
                "TMS" in r or "Si(C)(C)C" in r for r in reactants
            ...
            if (
                has_ketone_in_reactants and has_alkyne_in_reactants
            ) or has_propargyl_halide:
                # Check for propargyl alcohol pattern in product
                product_mol = Chem.MolFromSmiles(product)
                if product_mol:
                    propargyl_alcohol_pattern = Chem.MolFromSmarts(
                        "[#6](#[#6])[#6]([OH])"
                    )
