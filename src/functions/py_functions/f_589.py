...
def main(route):
        ...
        if (
            ...
            if has_isoxazole_in_product or backup_isoxazole_check:
                ...
                if not reactants_have_isoxazole:
                    ...
                    huisgen_1_3 = checker.check_reaction(
                    ...
                    huisgen_cu = checker.check_reaction(
                        "{Huisgen_Cu-catalyzed_1,4-subst}", rsmi
                    )
                    huisgen_ru = checker.check_reaction(
                        "{Huisgen_Ru-catalyzed_1,5_subst}", rsmi
                    )
                    huisgen_disubst = checker.check_reaction(
                        "{Huisgen_disubst-alkyne}", rsmi
                    )
                    ...
                    if product_ring_count > 1 and is_isoxazole_formation:
                        ...
                        print(
                            f"Multi-ring isoxazole formation confirmed at depth {depth}"
                        )
    ...
    if isoxazole_formed and depth_of_formation == 0:
        print(
            "Late-stage isoxazole formation in multi-ring system confirmed (at target molecule)"
        )
        return True

    print(
        f"Late-stage isoxazole formation not detected. Formation depth: {depth_of_formation}"
    )
    return False
