        ...
        if (
            ...
            for ring in heterocycle_types:
                if not checker.check_ring(
                    ring, reactants_smiles
                ) and checker.check_ring(ring, product_smiles):
            ...
            if not heterocycle_formation:
                for ring in heterocycle_types:
                    ...
                    if checker.check_reaction(reaction_name, rxn_smiles):
                        print(
                            f"Found heterocycle formation reaction for {ring}: {rxn_smiles}"
                        )
                        heterocycle_formation = True
                        break
            ...
            for rxn_type in coupling_reactions:
                if checker.check_reaction(rxn_type, rxn_smiles):
                    print(
                        f"Found potential biaryl coupling reaction ({rxn_type}): {rxn_smiles}"
                    )
                    ...
                    if product and all(r is not None for r in reactants):
                        ...
                        product_ring_atoms = [
                            ...
                            if all(
                                product.GetAtomWithIdx(i).GetIsAromatic() for i in ring
                            )
                        ]
                        ...
                        for bond in product.GetBonds():
                            ...
                            begin_rings = [
                                i
                                for i, ring in enumerate(product_ring_atoms)
                                if begin_atom in ring
                            ]
                            end_rings = [
                                i
                                for i, ring in enumerate(product_ring_atoms)
                            ...
                            if (
                                ...
                                print(
                                    f"Confirmed biaryl bond in product: {begin_atom}-{end_atom}"
                                )
