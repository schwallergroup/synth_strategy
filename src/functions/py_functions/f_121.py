...
def main(route):
            ...
            try:
                ...
                if any(r in phthalimide_deprotected_molecules for r in reactants):
                    ...
                    if not (
                        ...
                        print(
                            f"Found functionalization after phthalimide deprotection: {rsmi}"
                        )
    ...
    has_both_protecting_groups = (
        len(molecules_with_phthalimide) > 0 and len(molecules_with_cbz) > 0
    )
    ...
    if (
        has_both_protecting_groups
        and has_selective_deprotection
        and has_functionalization
    ):
    ...
    if len(molecules_with_cbz) > 0:
        for mol_smiles in molecules_with_cbz:
            if "CN1C(=O)c2ccccc2C1=O" in mol_smiles:
                print(
                    f"Found molecule with both phthalimide and Cbz (special case): {mol_smiles}"
                )
                return True
