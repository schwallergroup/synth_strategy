            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                if any(
                    ...
                    if (
                        ...
                        or checker.check_reaction(
                            "Heck reaction with vinyl ester", rsmi
                        )
                    ):
                        transformation_sequence.append(("halide_to_vinyl", depth))
                        print(
                            f"Confirmed halide to vinyl transformation at depth {depth}"
                        )
                    else:
                        ...
                        for r in reactants:
                            if checker.check_fg("Aromatic halide", r):
                                transformation_sequence.append(
                                    ("halide_to_vinyl", depth)
                                )
                                print(
                                    f"Added halide to vinyl transformation at depth {depth}"
                                )
                                break

                # Check for vinyl to aldehyde transformation
                if any(
                    checker.check_fg("Vinyl", r) for r in reactants
                ) and checker.check_fg("Aldehyde", product):
                    print(
                        f"Found reactant with vinyl and product with aldehyde at depth {depth}"
                    )
                    ...
                    if checker.check_reaction(
                        ...
                        print(
                            f"Confirmed vinyl to aldehyde transformation at depth {depth}"
                        )
                    else:
                        # More general check if specific reaction type not detected
                        transformation_sequence.append(("vinyl_to_aldehyde", depth))
                        print(
                            f"Added vinyl to aldehyde transformation at depth {depth}"
                        )
                ...
                if any(checker.check_fg("Aldehyde", r) for r in reactants):
                    ...
                    if (
                        ...
                        print(
                            f"Confirmed aldehyde to condensation transformation at depth {depth}"
                        )
                    else:
                        ...
                        if product_mol:
                            for bond in product_mol.GetBonds():
                                if bond.GetBondType() == Chem.BondType.DOUBLE:
                                    a1 = product_mol.GetAtomWithIdx(
                                        bond.GetBeginAtomIdx()
                                    )
                                    a2 = product_mol.GetAtomWithIdx(
                                        bond.GetEndAtomIdx()
                                    )
                                    if (
                                        a1.GetSymbol() == "C"
                                        and a2.GetSymbol() in ["C", "N"]
                                    ) or (
                                        a2.GetSymbol() == "C"
                                        and a1.GetSymbol() in ["C", "N"]
                                    ):
                                        transformation_sequence.append(
                                            ("aldehyde_to_condensation", depth)
                                        )
