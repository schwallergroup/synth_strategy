    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol" and "smiles" in node:
            ...
            if checker.check_ring("piperazine", mol_smiles):
                print(
                    f"Found piperazine scaffold at depth {depth} in molecule: {mol_smiles}"
                )
                has_piperazine = True
            ...
            # Check for piperazine-like structures (including piperazinone)
                ...
                if mol and mol.HasSubstructMatch(piperazinone_pattern):
                    print(
                        f"Found piperazinone scaffold at depth {depth} in molecule: {mol_smiles}"
                    )
                    has_piperazine = True
            ...
            # Check for piperazine-like structures in product
                ...
                if mol and mol.HasSubstructMatch(piperazinone_pattern):
                    print(
                        f"Found piperazinone scaffold in reaction product at depth {depth}"
                    )
                    has_piperazine = True
