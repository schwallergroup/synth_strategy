    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol" and node["smiles"]:
            ...
            if node.get("in_stock", False):
                print(
                    f"Depth {depth}: Starting material (skipping cyano check): {node['smiles']}"
                )
                return
            ...
            if not has_cyano:
                ...
                print(
                    f"Depth {depth}: Molecule without cyano group found: {node['smiles']}"
                )
                return
            ...
            try:
                ...
                if not checker.check_fg("Nitrile", product):
                    cyano_persists = False
                    print(
                        f"Depth {depth}: Reaction product doesn't have cyano group: {product}"
                    )
                    return

                # Check if at least one reactant has cyano group
                reactant_has_cyano = any(
                    checker.check_fg("Nitrile", r) for r in reactants
                )
                if not reactant_has_cyano:
                    cyano_persists = False
                    print(
                        f"Depth {depth}: No reactant has cyano group, but product does: {rsmi}"
                    )
                    return
