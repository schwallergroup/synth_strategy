...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if (
            ...
            if (
                product_mol
                and any(
                    m and m.HasSubstructMatch(halogen_aromatic_pattern)
                    for m in reactant_mols
                )
                and any(
                    m and m.HasSubstructMatch(cyclopropylamine_pattern)
                    for m in reactant_mols
                )
    ...
    if (
        found_cyclopropylamine_depth is not None
        and found_halogen_aromatic_depth is not None
    ):
        if (
            found_cyclopropylamine_depth >= 4
        ):  # Early stage (high depth in retrosynthesis)
            print(
