...
def main(route):
                ...
                try:
                    ...
                    if all(reactants_mols) and product_mol:
                        reactants_ring_count = sum(
                            ...
                            for m in reactants_mols
                        )
                        product_ring_count = Chem.rdMolDescriptors.CalcNumRings(
                            product_mol
                        )
                        ...
                        if product_ring_count > reactants_ring_count:
                            ...
                            oxime_involved = any(
                                checker.check_fg("Oxime", r)
                                for r in reactants_part.split(".")
                            )
                            if oxime_involved:
                                print(
                                    f"Found ring formation involving oxime at depth {depth}"
                                )
                                ring_formation_after_oxime = True
                                if (
                                    ring_formation_depth == -1
                                    or depth < ring_formation_depth
                                ):
    ...
    print(
        f"Ring formation found: {ring_formation_after_oxime} at depth {ring_formation_depth}"
    )
