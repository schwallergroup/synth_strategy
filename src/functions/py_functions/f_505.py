    ...
    def dfs_traverse(node):
        ...
        if (
            ...
            reactant_has_ester = any(
                Chem.MolFromSmiles(r).HasSubstructMatch(ester_pattern)
                for r in reactants
            )
            reactant_has_amide = any(
                Chem.MolFromSmiles(r).HasSubstructMatch(amide_pattern)
                for r in reactants
            )

            product_has_acid = Chem.MolFromSmiles(product).HasSubstructMatch(
                acid_pattern
            )
            product_has_ester = Chem.MolFromSmiles(product).HasSubstructMatch(
                ester_pattern
            )
            product_has_amide = Chem.MolFromSmiles(product).HasSubstructMatch(
                amide_pattern
            )
