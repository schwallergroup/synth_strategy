        ...
        if node["type"] == "reaction":
            try:
                ...
                if any(
                    ...
                    for rxn in carbonyl_forming_reactions
                ):
                    print(
                        f"Carbonyl-forming reaction detected at depth {depth}: {rsmi}"
                    )
                    carbonyl_formed = True

                # Check for carbonyl groups in product and reactants
                product_has_carbonyl = any(
                    checker.check_fg(fg, product) for fg in carbonyl_fgs
                )
                ...
                reactant_carbonyl_count = sum(
                    sum(1 for fg in carbonyl_fgs if checker.check_fg(fg, r))
                    for r in reactants
                )
                ...
                if not carbonyl_formed:
                    ...
                    if reactants_have_aldehyde_or_ketone and product_has_alcohol:
                        print(
                            f"Alcohol oxidation to carbonyl detected at depth {depth}: {rsmi}"
                        )
                        carbonyl_formed = True
                    ...
                    if reactants_have_aldehyde_or_ketone and product_has_alkyne:
                        print(
                            f"Alkyne hydration to carbonyl detected at depth {depth}: {rsmi}"
                        )
                        carbonyl_formed = True
