...
def main(route):
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if len(significant_reactants) <= 1:
                    ...
                    if atom_counts and len(atom_counts) > 1:
                        ...
                        others_max = max(
                            [c for c in atom_counts if c != max_atoms] or [0]
                        )
                        if max_atoms > 2 * others_max:
                            linear_reaction_count += 1
                            print(
                                f"Linear reaction step detected (size disparity): {rsmi}"
                            )
    ...
    if reaction_count > 0:
        ...
        print(
            f"Linear reaction ratio: {linear_ratio} ({linear_reaction_count}/{reaction_count})"
        )
