...
def main(route):
    ...
    def is_coupling_reaction(rsmi):
        ...
        reactants = [
            r for r in reactants_part.split(".") if not is_reagent_or_catalyst(r)
        ]
        if len(reactants) == 2:
    ...
    def dfs_traverse(node):
        if node["type"] == "reaction":
            ...
            significant_reactants = [
                r for r in all_reactants if not is_reagent_or_catalyst(r)
            ]
    ...
    coupling_percentage = (
        coupling_reactions.count(True) / len(coupling_reactions)
    ...
    is_linear = (
        has_coupling_reactions
        and coupling_percentage >= 0.6
        and two_fragment_percentage >= 0.6
    )
