...
def main(route):
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                if has_bicyclic_core:
                    ...
                    if not core_in_reactants:
                        print(
                            f"Found core-forming reaction for {core_type} at depth {depth}"
                        )
                            ...
                            for reactant in reactants:
                                if checker.check_ring(core_type, reactant):
                                    ...
                                    new_fgs = [
                                        ...
                                        if fg not in reactant_fgs
                                    ]
                                    if new_fgs:
                                        is_functionalization = True
                                        reaction_type = (
                                            f"Addition of {', '.join(new_fgs)}"
                                        )
                                        break
    ...
    print(
        f"Found {total_modifications} modifications and {len(core_formation)} core formations"
    )

    if total_modifications < 2 and not (
        has_core_formation and total_modifications >= 1
    ):
    ...
    if total_modifications >= 2:
        ...
        for i in range(len(depths) - 1):
            if (
                depths[i + 1] - depths[i] <= 3
            ):  # Allow up to 3 steps between modifications
                print(
                    f"Found sequential modifications at depths {depths[i]} and {depths[i+1]}"
                )
                return True
