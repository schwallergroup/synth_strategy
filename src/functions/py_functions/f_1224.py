...
def main(route):
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                if product_mol:
                    ...
                    if product_mol.HasSubstructMatch(boc_pattern):
                        ...
                        if not any(
                            r and r.HasSubstructMatch(boc_pattern)
                    ...
                    if product_mol.HasSubstructMatch(sulfonyl_pattern):
                        ...
                        reactant_mols = [Chem.MolFromSmiles(r) for r in reactants if r]
                        if not any(
                            r and r.HasSubstructMatch(sulfonyl_pattern)
                            for r in reactant_mols
                        ):
    ...
    if sequential_protection:
        print(
            "Detected sequential nitrogen protection strategy: Boc followed by sulfonylation"
        )
