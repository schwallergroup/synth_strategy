...
def main(route):
    ...
    def dfs_traverse(node):
        ...
        if (
            ...
            if (
                ...
                if any(
                    r and r.GetSubstructMatches(carbonyl_pattern) for r in reactant_mols
                ):
                    if len(product_mol.GetSubstructMatches(carbonyl_pattern)) < sum(
                        len(r.GetSubstructMatches(carbonyl_pattern))
    ...
    print(
        f"Multiple ester reductions strategy detected: {result} (count: {ester_reduction_count})"
    )
    return result
