    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            ...
            try:
                ...
                product_has_nitrile = (
                    checker.check_fg("Nitrile", product_smiles)
                    if product_smiles
                    else False
                )
                ...
                if reactants_have_nitrile and not product_has_nitrile:
                    ...
                    if (
                        ...
                        or checker.check_reaction(
                            "Grignard from nitrile to ketone", rsmi
                        )
                    ):
                        has_nitrile_transformation = True
                        print(
                            f"Detected specific nitrile transformation in reaction: {rsmi}"
                        )
                    else:
                        # Generic check for nitrile disappearance
                        has_nitrile_transformation = True
                        print(
                            f"Detected potential nitrile transformation in reaction: {rsmi}"
                        )
