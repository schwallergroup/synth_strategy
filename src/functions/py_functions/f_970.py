...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            ...
            if (
                len(product_rings) != len(reactant_rings)
                or product_rings != reactant_rings
            ):
                ...
                if (
                    ...
                    for reactant in reactants:
                        for ring in n_heterocycles:
                            if checker.check_ring(ring, reactant):
                                print(
                                    f"Detected heterocycle-modifying reaction at depth {depth}"
                                )
                                modified_heterocycles = True
                                break
            except Exception as e:
    ...
    if condition1 and condition2 and not condition3:
        ...
        if reaction_count >= 3:
            found_n_rich_synthesis = True
            print(
                f"Found nitrogen-rich heterocycle synthesis based on route complexity"
            )
