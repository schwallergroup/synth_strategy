...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "reaction":
            ...
            if (
                product and len(reactants) >= 2
            ):  # Need at least two reactants for fragment coupling
                ...
                acid_reactant = any(
                    mol.HasSubstructMatch(acid_pattern) for mol in reactants if mol
                )
                ester_reactant = any(
                ...
                if (
                    (acid_reactant or ester_reactant)
                    and amine_reactant
