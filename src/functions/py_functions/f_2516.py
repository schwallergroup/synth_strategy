...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        if node["type"] == "mol" and node["smiles"]:
            ...
            if depth == 0:
                ...
                if (
                    mol.GetNumHeavyAtoms() > 10
                ):  # Main pathway molecules are typically larger
                    main_pathway_molecules.append(
                        {
                            "smiles": smiles,
                            "has_chlorobenzene": checker.check_fg(
                    ...
                    print(
                        f"  Has chlorobenzene: {main_pathway_molecules[-1]['has_chlorobenzene']}"
                    )
                    print(
                        f"  Has phenyl ether: {main_pathway_molecules[-1]['has_phenyl_ether']}"
                    )
    ...
    if not (
        target_molecule["has_chlorobenzene"] and target_molecule["has_phenyl_ether"]
    ):
        print("Target molecule doesn't have both motifs")
        return False

    # Check if all main pathway molecules have both motifs
    all_have_chlorobenzene = all(
        mol["has_chlorobenzene"] for mol in main_pathway_molecules
    )
    all_have_phenyl_ether = all(
        mol["has_phenyl_ether"] for mol in main_pathway_molecules
    )
