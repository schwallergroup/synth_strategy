...
def main(route):
        ...
        if node["type"] == "reaction":
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if product and any(reactant for reactant in reactants):
                    ...
                    product_has_nitro = product and product.HasSubstructMatch(
                        nitro_pattern
                    )
    ...
    strategy_present = (
        reactions_with_nitro >= 3 and reactions_with_nitro == total_reactions
    )

    print(
        f"Pyridine elaboration with preserved nitro strategy detected: {strategy_present}"
    )
    print(
        f"- Reactions with nitro preservation: {reactions_with_nitro}/{total_reactions}"
    )
