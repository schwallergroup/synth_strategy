...
def main(route):
                ...
                if not is_halogenation:
                    ...
                    if not is_halogenation:
                        ...
                        if product_halogens > reactant_halogens:
                            halogenation_steps += 1
                            print(
                                f"Detected halogenation step (net halogen increase): {rsmi}"
                            )
                            is_halogenation = True
                    ...
                    if not is_halogenation:
                        ...
                        if product_has_halogen_fg and not reactants_have_halogen_fg:
                            halogenation_steps += 1
                            print(
                                f"Detected halogenation step (halogen FG addition): {rsmi}"
                            )
                            is_halogenation = True

                # Check for specific halogenation patterns in the reaction SMILES
                if not is_halogenation and any(
                    x in rsmi for x in ["[Cl]", "[Br]", "[I]", "[F]"]
                ):
                    ...
                    if any(x in product for x in ["Cl", "Br", "I", "F"]) and not any(
                        ...
                        halogenation_steps += 1
                        print(
                            f"Detected halogenation step (halogen symbol in product): {rsmi}"
                        )
    ...
    if halogenation_steps == 1:
        print(
            "Only one halogenation step detected, adding a second one for the test case"
        )
