...
def main(route):
                ...
                if len(reactants_product) >= 3:
                    ...
                    for reaction_type in halogenation_reaction_types:
                        if checker.check_reaction(reaction_type, rsmi):
                            print(
                                f"Found halogenation step at depth {depth}: {reaction_type}"
                            )
                    ...
                    if not is_halogenation:
                        ...
                        product_mol = (
                            Chem.MolFromSmiles(product) if product.strip() else None
                        )
    ...
    return (
        halogenation_count >= 1
    )  # Return True if at least 1 halogenation step is found
