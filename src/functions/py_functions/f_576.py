...
def main(route):
        ...
        if node["type"] == "reaction":
            ...
            if product_mol and all(reactants_mols):
                ...
                if any(
                    ...
                    print(
                        f"Detected methyl → bromomethyl transformation at depth {depth}"
                    )
    ...
    for i in range(len(expected_sequence) - 1):
        if (
            ...
            if transformation_sequence.index(
                expected_sequence[i]
            ) > transformation_sequence.index(expected_sequence[i + 1]):
                return False
    ...
    if has_sequence:
        print(
            f"Detected functional group transformation sequence: {transformation_sequence}"
        )
