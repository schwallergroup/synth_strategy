    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol" and "smiles" in node:
            ...
            if checker.check_ring("dioxolane", mol_smiles):
                print(
                    f"Found dioxolane ring (potential acetonide) at depth {depth}: {mol_smiles}"
                )
                acetonide_present = True

            if checker.check_ring("dioxane", mol_smiles):
                print(
                    f"Found dioxane ring (potential acetonide) at depth {depth}: {mol_smiles}"
                )
                acetonide_present = True
        ...
        # Check reaction nodes for acetonide formation/hydrolysis
            ...
            if checker.check_reaction("Diol acetalization", rxn_smiles):
                print(
                    f"Found diol acetalization reaction at depth {depth}: {rxn_smiles}"
                )
            ...
            if checker.check_reaction("Acetal hydrolysis to diol", rxn_smiles):
                print(
                    f"Found acetal hydrolysis to diol reaction at depth {depth}: {rxn_smiles}"
                )
