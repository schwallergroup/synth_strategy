...
def main(route):
            ...
            if not reaction_detected:
                ...
                for reactant in reactants:
                    ...
                    for fg in [
                        ...
                        if checker.check_fg(fg, reactant) and not checker.check_fg(
                            fg, product
                        ):
                            ...
                            for new_fg in [
                                ...
                                if not checker.check_fg(
                                    new_fg, reactant
                                ) and checker.check_fg(new_fg, product):
                ...
                if not reaction_detected:
                    for fg in [
                        ...
                        if checker.check_fg(fg, product) and not any(
                            ...
                            functionalization_steps.append(
                                (f"{fg.lower()}_formation", depth)
                            )
                            print(
                                f"Detected {fg.lower()} formation at depth {depth}: {rsmi}"
                            )
                            reaction_detected = True
                            break
    ...
    if len(unique_steps) >= 4:
        ...
        if is_sequential:
            ...
            print(
                f"Found {len(unique_steps)} functionalization steps, but they are not sequential"
            )
    else:
        print(
            f"Found only {len(unique_steps)} different functionalization steps, need at least 4"
        )
