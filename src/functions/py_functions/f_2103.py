...
def main(route):
    ...
    if len(stereocenters_by_mol) >= 2:
        ...
        mols_by_depth = sorted(
            [(depth, smiles) for smiles, depth in depth_by_mol.items()]
        )
        ...
        if len(mols_by_depth) >= 2:
            ...
            if max_depth - min_depth >= 2:
                ...
                early_mols = [
                    smiles for depth, smiles in mols_by_depth if depth >= max_depth - 1
                ]
                late_mols = [
                    smiles for depth, smiles in mols_by_depth if depth <= min_depth + 1
                ]
