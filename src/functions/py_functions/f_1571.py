            ...
            try:
                ...
                for reaction_type in heterocycle_reactions:
                    if checker.check_reaction(reaction_type, rsmi):
                        print(
                            f"Detected heterocycle formation reaction: {reaction_type}"
                        )
                ...
                if product_mol and all(reactant_mols):
                    ...
                    if new_heterocycles:
                        print(
                            f"Detected new heterocycle(s) in product: {new_heterocycles}"
                        )
                        late_stage_ring_formation = True
                        return
                    ...
                    if product_ring_count != reactant_ring_count:
                        ...
                        for ring in Chem.GetSymmSSSR(product_mol):
                            ...
                            if has_heteroatom:
                                print(
                                    f"Detected heterocycle modification at depth {depth}"
                                )
                                late_stage_ring_formation = True
                                return
