...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if (
        ...
        ):  # Late stage (within first few reactions)
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if reactant_has_ketone and product_has_alcohol:
                    print(
                        f"Found potential ketone to alcohol transformation at depth {depth}"
                    )
                    # Try to verify this is a reduction reaction
                    if (
                        not found_late_oxidation
                    ):  # Only check if we haven't found it yet
                        found_late_oxidation = True
                        print(
                            f"Confirmed functional group transformation at depth {depth}"
                        )
                        return
