...
def main(route):
    ...
    def has_aromatic_alkyne_connection(mol_smiles):
        ...
        if mol.HasSubstructMatch(pattern1):
            ...
            print(
                f"Found aromatic-linker-alkyne-linker-aromatic connection in: {mol_smiles}"
            )
            return True
    ...
    if route["type"] == "mol" and not has_aromatic_alkyne_connection(route["smiles"]):
        print(
            f"Target molecule doesn't have aromatic-alkyne-aromatic linker: {route['smiles']}"
        )
        return False
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            ...
            if has_aromatic_alkyne_connection(smiles):
                print(
                    f"Found aromatic-alkyne-aromatic linker at depth {depth}: {smiles}"
                )
                ...
                for reactant in reactants:
                    if has_aromatic_alkyne_connection(reactant):
                        if not has_aromatic_alkyne_connection(product):
                            print(
                                f"Alkyne linker broken in reaction at depth {depth}: {rsmi}"
                            )
