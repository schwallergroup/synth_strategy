...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        if node["type"] == "reaction":
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if any(
                    m and m.HasSubstructMatch(carboxylic_pattern)
                ...
                if any(
                    m
                    and (
                        m.HasSubstructMatch(tbutyl_pattern)
                        or m.HasSubstructMatch(methyl_pattern)
                    )
                    for m in reactant_mols
                    if m
                ):
    ...
    print(
        f"Sequential protection-deprotection strategy detected: {sequential_strategy}"
    )
