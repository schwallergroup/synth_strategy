...
def main(route):
    ...
    def is_simple_reagent(smiles):
        ...
        for pattern in common_patterns:
            if Chem.MolFromSmiles(pattern) and Chem.MolFromSmiles(
                pattern
            ).GetSubstructMatch(mol):
                return True
    ...
    if linear_steps and convergent_steps:
        ...
        if final_step and final_step in [step[:2] for step in convergent_steps]:
            ...
            linear_before_convergent = [
                step for step in linear_steps if step[0] > final_step[0]
            ]
    ...
    print(
        f"Linear steps: {len(linear_steps)}, Convergent steps: {len(convergent_steps)}"
    )
