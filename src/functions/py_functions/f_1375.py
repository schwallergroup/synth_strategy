        ...
        if node["type"] == "mol" and "smiles" in node:
            ...
            if depth == 0 or (node.get("children") and len(node.get("children")) > 0):
                ...
                if not has_difluoromethoxy:
                    ...
                    has_difluoromethoxy = mol and mol.HasSubstructMatch(
                        difluoromethoxy_pattern
                    )

                if not has_difluoromethoxy:
                    all_steps_have_difluoromethoxy = False
                    print(
                        f"No difluoromethoxy group found in main pathway molecule: {mol_smiles}"
                    )
                else:
                    print(
                        f"Found difluoromethoxy group in main pathway molecule: {mol_smiles}"
                    )
