                        ...
                        if product_mol and tetrazole_reactant_mol:
                            ...
                            for atom in tetrazole_reactant_mol.GetAtoms():
                                if atom.GetSymbol().lower() == "n" and atom.HasProp(
                                    ...
                                    print(
                                        f"Found N atom in tetrazole reactant with map {map_num}"
                                    )
                            ...
                            for _, map_num in tetrazole_n_atoms:
                                ...
                                if reactant_n_atom and product_n_atom:
                                    ...
                                    for neighbor in product_n_atom.GetNeighbors():
                                        if neighbor.GetSymbol() == "C":
                                            ...
                                            if neighbor.HasProp("molAtomMapNumber"):
                                                ...
                                                for (
                                                    alkyl_atom
                                                ) in alkyl_halide_mol.GetAtoms():
                                                    if (
                                                        alkyl_atom.HasProp(
                                                            "molAtomMapNumber"
                                                        )
                                                        and alkyl_atom.GetProp(
                                                            "molAtomMapNumber"
                                                        )
                                                        == c_map_num
                                                    ):
                                    ...
                                    if (
                                        ...
                                        print(
                                            "Confirmed: NH in tetrazole was alkylated"
                                        )
                                        found = True
                                        break
                            ...
                            if (
                                not found
                                and has_tetrazole_reactant
                                and has_alkyl_halide
                            ):
                                # Check if the tetrazole in product has a new alkyl group
                                # This is a fallback method
                                tetrazole_indices_reactant = (
                                    checker.get_ring_atom_indices(
                                        "tetrazole", tetrazole_reactant
                                    )
                                )
                                tetrazole_indices_product = (
                                    checker.get_ring_atom_indices("tetrazole", product)
                                )

                                if (
                                    tetrazole_indices_reactant
                                    and tetrazole_indices_product
                                ):
                                    print(
                                        "Tetrazole found in both reactant and product"
                                    )
