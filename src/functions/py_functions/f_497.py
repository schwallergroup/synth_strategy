    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "reaction":
            ...
            for reactant in reactants_smiles:
                ...
                if (
                ...
                ):  # Hydroxylamine and derivatives are small
                    ...
                    if any(pattern in reactant for pattern in hydroxylamine_patterns):
                        print(
                            f"Potential hydroxylamine derivative detected: {reactant}"
                        )
                    ...
                    if checker.check_fg("Primary amine", reactant) and "O" in reactant:
                        n_count = sum(
                            1 for atom in mol.GetAtoms() if atom.GetSymbol() == "N"
                        )
                        o_count = sum(
                            1 for atom in mol.GetAtoms() if atom.GetSymbol() == "O"
                        )
