...
def main(route):
        ...
        if node["type"] == "reaction":
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if reactant_mol:
                    ...
                    protected_aniline_pattern = Chem.MolFromSmarts(
                        "[c]-[N]-[C](=[O])-[O]"
                    )
    ...
    is_linear_with_protected_aniline = (
        steps_with_protected_aniline >= 2 and total_steps >= 3
    )

    print(f"Steps with protected aniline: {steps_with_protected_aniline}/{total_steps}")
    print(
        f"Linear synthesis with protected aniline: {is_linear_with_protected_aniline}"
    )
