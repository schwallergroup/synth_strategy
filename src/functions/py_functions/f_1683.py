    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "reaction" and node.get("metadata", {}).get("rsmi"):
            ...
            if depth <= 1:  # Final or penultimate step (late-stage)
                ...
                product_has_sulfonamide = checker.check_fg(
                ...
                if product_has_sulfonamide:
                    ...
                    for reactant in reactants_smiles:
                        if checker.check_fg("Sulfonamide", reactant):
                            ...
                            print(
                                f"Sulfonamide already present in reactant: {reactant}"
                            )
                            break
                    ...
                    if not sulfonamide_in_reactants and (
                        ...
                        or (sulfonyl_chloride_present and amine_present)
                    ):
                        print(
                            f"Detected late-stage sulfonamide formation at depth {depth}"
                        )
                        result = True
                else:
