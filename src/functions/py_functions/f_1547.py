...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            ...
            if depth == 0:
                ...
                if (
                    checker.check_fg("Trifluoro group", mol_smiles)
                    or "C(F)F" in mol_smiles
                ):
    ...
    if len(transformation_sequence) >= 3:
        ...
        if nitrile_idx != -1 and chloride_idx != -1 and aldehyde_idx != -1:
            if nitrile_idx < chloride_idx < aldehyde_idx:
                ...
                print(
                    "Transformations occurred in the correct sequence (retrosynthetic order)"
                )
    elif (
        len(transformation_sequence) == 1
        and transformation_sequence[0] == "chloride_to_nitrile"
    ):
    ...
    print(
        f"- Benzylic functionalization sequence: {benzylic_functionalization_sequence}"
    )
