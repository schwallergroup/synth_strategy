            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                for rxn in amide_formation_reactions:
                    if checker.check_reaction(rxn, rsmi):
                        ...
                        has_acid_precursor = any(
                            any(checker.check_fg(fg, r) for fg in acid_groups)
                            for r in reactants
                        )

                        has_amine = any(
                            any(checker.check_fg(fg, r) for fg in amine_groups)
                            for r in reactants
                        )
                        ...
                        reactants_have_amide = any(
                            any(checker.check_fg(fg, r) for fg in amide_groups)
                            for r in reactants
                        )
                ...
                if not has_amide_formation:
                    ...
                    if (
                        ...
                        print(
                            "Detected amide formation through functional group analysis"
                        )
                        has_amide_formation = True
