...
def main(route):
    ...
    preserved_through_route = [
        False
    ]  # Using list for mutable reference in nested function
    ...
    def dfs_traverse(node, path=None):
        ...
        if node["type"] == "mol":
            ...
            if len(path) == 0 and has_cyclopropane:
                ...
                path.append(
                    {"has_cyclopropane": has_cyclopropane, "smiles": node["smiles"]}
                )
            elif has_cyclopropane:
                print(
                    f"Intermediate at depth {len(path)} has cyclopropane: {node['smiles']}"
                )
                path.append(
                    {"has_cyclopropane": has_cyclopropane, "smiles": node["smiles"]}
                )
                ...
                print(
                    f"Molecule at depth {len(path)} does NOT have cyclopropane: {node['smiles']}"
                )
