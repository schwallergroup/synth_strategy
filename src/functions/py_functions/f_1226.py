...
def main(route):
        ...
        if node["type"] == "mol":
            ...
            if is_main_pathway:
                ...
                if has_boc:
                    ...
                    print(
                        f"Depth {depth}: Molecule without Boc protection: {mol_smiles}"
                    )
            ...
            try:
                ...
                reactants_have_boc = (
                    all(
                        checker.check_fg("Boc", reactant) for reactant in main_reactants
                    )
                    if main_reactants
                    else False
                )
                ...
                if is_main_pathway and (not product_has_boc or not reactants_have_boc):
                    if not product_has_boc:
                        print(
                            f"Depth {depth}: Product without Boc protection: {product}"
                        )
                    if not reactants_have_boc:
                        print(
                            f"Depth {depth}: Not all main reactants have Boc protection"
                        )
    ...
    print(
        f"All molecules in main pathway have Boc protection across {reaction_count} reactions"
    )
    return True
