...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol" and "smiles" in node:
            ...
            ) or checker.check_fg("TMS ether protective group", mol_smiles):
        ...
        if (
            ...
            if not has_protection:
                ...
                if any(
                    "C(=O)OC(C)(C)C" in r for r in reactants
                ) or "OC(=O)OC(C)(C)C" in "".join(reactants):
                    if "[NH" in "".join(reactants) and "NC(=O)OC(C)(C)C" in product:
                        print(
                            f"{indent}Found Boc protection reaction by pattern matching"
                        )
                        has_protection = True
                ...
                if (
                    ...
                    if not any(
                        ...
                        print(
                            f"{indent}Found potential amide protection by pattern matching"
                        )
                        has_protection = True
            ...
            if not has_cross_coupling:
                ...
                halide_present = any(
                    ("I" in r or "Br" in r or "Cl" in r) for r in reactants
                )
            ...
            if not has_snar:
                ...
                halide_on_aromatic = any(
                    ("Cl" in r or "F" in r or "Br" in r or "I" in r)
                    and ("c" in r or "n" in r)
                    for r in reactants
                )
                nucleophile_present = any(
                    ("[NH" in r or "[OH" in r or "[SH" in r or "NH3" in r)
                    for r in reactants
                )
