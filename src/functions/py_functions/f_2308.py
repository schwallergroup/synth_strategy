...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        if (
            ...
            if (
                ...
                or checker.check_reaction(
                    "Reductive amination with aldehyde", rxn_smiles
                )
                ...
                print(
                    f"N-alkylation reaction found at depth {depth}: {rxn_smiles[:50]}..."
                )
    ...
    if n_alkylation_reactions:
        ...
        if latest_alkylation_depth <= 3:
            print(
                f"Late-stage N-alkylation detected at depth {latest_alkylation_depth}"
            )
            return True
        else:
            print(
                f"N-alkylation found but not late-stage (depth {latest_alkylation_depth})"
            )
        ...
        if route["type"] == "mol" and checker.check_fg(
            "Tertiary amine", route["smiles"]
        ):
            print(
                "Final product contains tertiary amine, suggesting N-alkylation occurred"
            )
            return True
