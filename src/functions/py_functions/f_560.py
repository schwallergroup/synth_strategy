...
def main(route):
    ...
    for depth, scaffold, smiles in molecule_scaffolds[1:]:
        ...
        if similarity < 0.8:  # Increased threshold for scaffold preservation
            print(
                f"Scaffold not preserved at depth {depth}, similarity: {similarity:.2f}"
            )
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                if not is_fg_transformation:
                    for reactant in reactants:
                        ...
                        # Compare scaffolds
                        prod_fp = AllChem.GetMorganFingerprintAsBitVect(
                            product_scaffold, 2
                        )
                        react_fp = AllChem.GetMorganFingerprintAsBitVect(
                            reactant_scaffold, 2
                        )
                        ...
                        if similarity < 0.8:
                            print(
                                f"Reaction modifies scaffold, similarity: {similarity:.2f}"
                            )
