...
def main(route):
    ...
    def identify_groups(node, depth=0):
        if node["type"] == "mol":
            ...
            if mol:
                ...
                has_fluoro = (
                    checker.check_fg("Aromatic halide", smiles) and "F" in smiles
                )
                if has_fluoro:
    ...
    def check_preservation(node, depth=0):
        ...
        if (
            ...
            product_has_methoxy = checker.check_fg(
                "Ether", product
            ...
            reactants_have_methoxy = any(
                checker.check_fg("Ether", r) and checker.check_ring("benzene", r)
                for r in reactants
            )
    ...
    final_has_methoxy = checker.check_fg(
        "Ether", final_product_smiles
    ) and checker.check_ring("benzene", final_product_smiles)
    final_has_fluoro = (
        checker.check_fg("Aromatic halide", final_product_smiles)
        and "F" in final_product_smiles
    )
