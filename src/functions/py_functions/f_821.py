...
def main(route):
    ...
    reaction_linearity = (
        linear_reactions / total_reactions if total_reactions > 0 else 0
    )
    ...
    print(
        f"Reaction linearity: {reaction_linearity:.2f} ({linear_reactions}/{total_reactions})"
    )
    print(
    ...
    is_linear = (
        reaction_linearity >= 0.8
        and molecule_non_branching >= 0.8
        and max_branching <= 2
    )
