...
def main(route):
        ...
        if (
            ...
            try:
                ...
                if checker.check_reaction("Sulfanyl to sulfinyl", rsmi):
                    print(
                        f"Found late-stage sulfide oxidation via reaction check: {rsmi}"
                    )
                    ...
                    has_sulfide_reactant = any(
                        checker.check_fg("Monosulfide", r)
                        and not checker.check_fg("Sulfoxide", r)
                        for r in reactants
                    )

                    # Check if product has a sulfoxide
                    has_sulfoxide_product = checker.check_fg(
                        "Sulfoxide", product_smiles
                    )

                    if has_sulfide_reactant and has_sulfoxide_product:
                        print(
                            f"Found late-stage sulfide oxidation via FG check: {rsmi}"
                        )
                        found_late_stage_oxidation = True
    ...
    print(
        f"Late-stage sulfoxide formation detection result: {found_late_stage_oxidation}"
    )
