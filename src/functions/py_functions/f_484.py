...
def main(route):
        ...
        if node["type"] == "reaction" and "rsmi" in node.get("metadata", {}):
            ...
            try:
                ...
                if checker.check_fg("Azide", product_smiles):
                    if not any(
                        checker.check_fg("Azide", reactant)
                        for reactant in reactants_smiles
                    ):
                        # In retrosynthesis, we're going backward, so this is where azide is introduced
                        if (
                            azide_introduction_depth is None
                            or depth < azide_introduction_depth
                        ):
    ...
    if azide_introduction_depth is not None:
        ...
        print(
            f"Max depth: {max_depth}, Azide introduction depth: {azide_introduction_depth}"
        )
        print(
            f"Azide introduction is {'late-stage' if is_late_stage else 'early-stage'}"
        )
