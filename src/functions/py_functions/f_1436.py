...
def main(route):
            ...
            # Otherwise, inherit branch from children
                ...
                if child_branches:
                    ...
                    return child_branches[
                        0
                    ]  # Return first branch for parent inheritance
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                if is_amide_coupling:
                    ...
                    if len(reactants) >= 2:
                        ...
                        if acid_found and amine_found and amide_in_product:
                            print(
                                "Confirmed acid/acyl and amine reactants with amide product"
                            )
                            ...
                            if len(complex_reactants) >= 2:
                                ...
                                if (
                                    ...
                                    if isinstance(acid_branch, list) or isinstance(
                                        ...
                                        if set(acid_branches) != set(
                                            amine_branches
                                        ) and not (
                                            set(acid_branches).issubset(
                                                set(amine_branches)
                                            )
                                            or set(amine_branches).issubset(
                                                set(acid_branches)
                                            )
                                        ):
                                            print(
    ...
    if not found_late_stage_amide:
        ...
        starting_materials = [
            smiles
            for smiles, branch in synthesis_branches.items()
            if isinstance(branch, int)
        ]

        # Check if we have at least one carboxylic acid and one amine in starting materials
        acid_starting = any(
            checker.check_fg("Carboxylic acid", sm) for sm in starting_materials
        )
        amine_starting = any(
        ...
        print(
            f"Starting materials contain acid: {acid_starting}, amine: {amine_starting}"
        )
        ...
        if (
            acid_starting
            and amine_starting
            and len(set(synthesis_branches.values())) >= 2
        ):
