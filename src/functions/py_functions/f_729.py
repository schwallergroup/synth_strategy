...
def main(route):
        ...
        if node["type"] == "reaction":
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if checker.check_reaction("Formation of Azides from halogens", rsmi):
                    print(
                        f"Found 'Formation of Azides from halogens' reaction at depth {depth}"
                    )
                        ...
                        if mol:
                            for atom in mol.GetAtoms():
                                if atom.GetAtomicNum() == 35:  # Bromine atomic number
                                    has_bromide = True
                                    print(
                                        f"Found bromide in reactant (general check): {reactant}"
                                    )
                                    break
                ...
                if not n3_to_nh2_detected:
                    ...
                    for reactant in reactants:
                        if checker.check_fg("Azide", reactant):
                            has_azide = True
                            print(
                                f"Found azide in reactant (general check): {reactant}"
                            )
                            break
    ...
    print(
        f"br_to_n3_detected: {br_to_n3_detected}, n3_to_nh2_detected: {n3_to_nh2_detected}"
    )
