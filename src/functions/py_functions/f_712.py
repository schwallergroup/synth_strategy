                ...
                try:
                    ...
                    if r_mol and r_mol.HasSubstructMatch(nitro_pattern):
                        ...
                        if p_mol:
                            ...
                            nitro_count_r = len(
                                r_mol.GetSubstructMatches(nitro_pattern)
                            )
                            # Count nitro groups in product
                            nitro_count_p = len(
                                p_mol.GetSubstructMatches(nitro_pattern)
                            )

                            # Check if nitro groups decreased and amines increased
                            amine_pattern = Chem.MolFromSmarts("[NH2]")
                            amine_count_r = len(
                                r_mol.GetSubstructMatches(amine_pattern)
                            )
                            amine_count_p = len(
                                p_mol.GetSubstructMatches(amine_pattern)
                            )

                            if (
                                nitro_count_p < nitro_count_r
                                and amine_count_p > amine_count_r
                            ):
