...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if depth == 0 and node["type"] == "mol":
            ...
            if not has_nitro_installation and checker.check_fg(
                "Nitro group", product_smiles
            ):
                has_nitro_installation = True
                nitro_depth = (
                    999  # High depth to indicate it's present from the beginning
                )
            ...
            try:
                ...
                if not has_nitro_installation:
                    ...
                    if (
                        ...
                        or checker.check_reaction(
                            "Non-aromatic nitration with HNO3", rsmi
                        )
                    ):
                        has_nitro_installation = True
                        nitro_depth = depth
                        print(
                            f"Detected nitro group installation via reaction at depth {depth}"
                        )
                    else:
                        # Fallback to functional group check
                        product_has_nitro = checker.check_fg(
                            "Nitro group", product_smiles
                        )
                ...
                if not has_methoxy_installation:
                    ...
                    if (
                        ...
                        if product_mol and all(r for r in reactant_mols):
                            methoxy_pattern = Chem.MolFromSmarts("c-[O;X2][C;X4;H3]")
                            product_has_methoxy = product_mol.HasSubstructMatch(
                                methoxy_pattern
                            )
                            reactants_have_methoxy = any(
                                r.HasSubstructMatch(methoxy_pattern)
                            ...
                            if product_has_methoxy and not reactants_have_methoxy:
                                ...
                                methoxy_depth = depth
                                print(
                                    f"Detected methoxy group installation at depth {depth}"
                                )
                ...
                if not has_carboxylic_derivatization:
                    ...
                    if (
                        ...
                        product_has_acyl = checker.check_fg(
                            "Acyl halide", product_smiles
                        )
                        ...
                        if (
                            ...
                            carboxylic_depth = depth
                            print(
                                f"Detected carboxylic acid derivatization at depth {depth}"
                            )
                ...
                if not has_benzylic_functionalization:
                    ...
                    if any(
                        ...
                        print(
                            f"Detected benzylic functionalization via reaction at depth {depth}"
                        )
                    else:
                                ...
                                for r_mol in reactant_mols:
                                    if r_mol.HasSubstructMatch(benzylic_pattern):
                                        ...
                                        if product_mol.HasSubstructMatch(
                                            benzylic_halide
                                        ) and not r_mol.HasSubstructMatch(
                                            benzylic_halide
                                        ):
                                            has_benzylic_functionalization = True
                                            benzylic_depth = depth
