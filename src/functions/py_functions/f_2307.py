    ...
    def dfs_traverse(node, depth=0):
        if (
            ...
            if (
                ...
                or checker.check_reaction(
                    "Acetic anhydride and alcohol to ester", rxn_smiles
                )
                ...
                print(
                    f"Ester formation reaction found at depth {depth}: {rxn_smiles[:50]}..."
                )
            ...
            if (
                ...
                or checker.check_reaction(
                    "Ester saponification (methyl deprotection)", rxn_smiles
                )
                or checker.check_reaction(
                    "Ester saponification (alkyl deprotection)", rxn_smiles
                )
                or checker.check_reaction("COOH ethyl deprotection", rxn_smiles)
            ):
                ester_hydrolysis_reactions.append((depth, rxn_smiles))
                print(
                    f"Ester hydrolysis reaction found at depth {depth}: {rxn_smiles[:50]}..."
                )
