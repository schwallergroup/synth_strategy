    ...
    def is_peg_chain(mol_smiles):
        ...
        try:
            ...
            if mol.HasSubstructMatch(peg_pattern):
                print(
                    f"Found PEG-like structure with {len(ether_indices)} ether groups"
                )
                return True
            ...
            for i, indices_i in enumerate(ether_indices):
                ...
                for j, indices_j in enumerate(ether_indices):
                    if i != j:
                        ...
                        if (
                            3 <= len(path) <= 4
                        ):  # Path length includes start and end atoms
                            connected_ethers += 1
                            break

            if connected_ethers >= MIN_ETHER_COUNT - 1:
                print(
                    f"Found PEG-like structure with {connected_ethers} connected ether groups"
                )
                return True
                    ...
                    if is_peg_chain(reactant_smiles):
                        ...
                        for product in products:
                            if checker.check_fg("Ether", product):
                                ...
                                print(
                                    f"Product contains {len(product_ether_indices)} ether groups"
                                )
                        ...
                        if alcohol_products >= 2:
                            print(
                                f"Multiple alcohol products from PEG chain cleavage detected"
                            )
                            has_peg_cleavage = True
                            return
                ...
                if len(products) == 1 and len(reactants) >= 2:
                    ...
                    if is_peg_chain(product_smiles):
                        ...
                        if checker.check_reaction("Williamson Ether Synthesis", rsmi):
                            print(
                                f"Detected ether formation reaction (retrosynthetic cleavage)"
                            )
                            has_peg_cleavage = True
                            return
                        ...
                        if alcohol_reactants >= 2 and ether_containing_reactants > 0:
                            print(
                                f"Multiple alcohol reactants forming PEG chain detected"
                            )
                            has_peg_cleavage = True
                            return
            except Exception as e:
