...
def main(route):
            ...
            try:
                ...
                if (
                    ...
                    or (
                        ...
                        and not all(
                            checker.check_fg("Ether", r) for r in reactants_smiles
                        )
                    )
                    or (
                        any(
                    ...
                    print(
                        f"Found linker installation at depth {depth}, reaction: {rsmi}"
                    )
                ...
                if all(
                    [product_mol] + reactant_mols
                ):  # Ensure all molecules parsed correctly
                    product_ring_count = len(Chem.GetSSSR(product_mol))
                    reactant_ring_count = sum(
                        len(Chem.GetSSSR(r)) for r in reactant_mols
                    )
                    ...
                    if product_ring_count > reactant_ring_count:
                        ...
                        for ring in heterocycle_rings:
                            if checker.check_ring(ring, product_smiles) and not any(
                                ...
                                print(
                                    f"Found heterocycle formation ({ring}) at depth {depth}"
                                )
                                break
                ...
                if any(checker.check_reaction(rxn, rsmi) for rxn in amine_reactions):
                    ...
                    print(
                        f"Found late-stage amine substitution at depth {depth}, reaction: {rsmi}"
                    )
                elif (
    ...
    print(
        f"Linker installation: {found_linker_installation} at depth {linker_installation_depth}"
    )
    print(
    ...
    if found_heterocycle_formation:  # At minimum, we need heterocycle formation
        ...
        # If we only found heterocycle formation and linker installation
            ...
            if linker_installation_depth > heterocycle_formation_depth:
                print(
                    "Found partial strategy: linker installation → heterocycle formation"
                )
                return True
    ...
    if (
        ...
        if linker_installation_depth > amine_substitution_depth:
            print(
                "Found alternative strategy: linker installation → late-stage amine substitution"
            )
            return True
