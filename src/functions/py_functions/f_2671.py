...
def main(route):
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                # Phenol to triflate conversion
                    ...
                    if checker.check_reaction("Formation of Sulfonic Esters", rsmi):
                        ...
                        print(
                            f"Found transformation at depth {depth}: phenol → triflate: {rsmi}"
                        )
                ...
                # Phenol to mesylate conversion
                    ...
                    if checker.check_reaction("Formation of Sulfonic Esters", rsmi):
                        ...
                        print(
                            f"Found transformation at depth {depth}: phenol → mesylate: {rsmi}"
                        )

                # Phenol to tosylate conversion
                elif any(
                    checker.check_fg("Phenol", r) for r in reactants
                ) and checker.check_fg("Tosylate", product):
                    if checker.check_reaction("Formation of Sulfonic Esters", rsmi):
                        transformations.append(("phenol_to_tosylate", depth))
                        print(
                            f"Found transformation at depth {depth}: phenol → tosylate: {rsmi}"
                        )

                # Triflate to boronic ester
                elif any(
                    checker.check_fg("Triflate", r) for r in reactants
                ...
                # Mesylate to boronic ester
                    ...
                    checker.check_fg("Mesylate", r) for r in reactants
                ) and checker.check_fg("Boronic ester", product):
                ...
                # Tosylate to boronic ester
                    ...
                    checker.check_fg("Tosylate", r) for r in reactants
                ) and checker.check_fg("Boronic ester", product):
                ...
                # Phenol to aryl halide
                    ...
                    print(
                        f"Found transformation at depth {depth}: phenol → halide: {rsmi}"
                    )
    ...
    direct_transformation = any(
        t[0] == "phenol_to_boronic_ester_direct" for t in transformations
    )

    # Check if we have a pathway through triflate
    triflate_pathway = any(
        t[0] == "phenol_to_triflate" for t in transformations
    ) and any(t[0] == "triflate_to_boronic_ester" for t in transformations)

    # Check if we have a pathway through mesylate
    mesylate_pathway = any(
        t[0] == "phenol_to_mesylate" for t in transformations
    ) and any(t[0] == "mesylate_to_boronic_ester" for t in transformations)

    # Check if we have a pathway through tosylate
    tosylate_pathway = any(
        t[0] == "phenol_to_tosylate" for t in transformations
    ) and any(t[0] == "tosylate_to_boronic_ester" for t in transformations)
    ...
    if target_has_boronic_ester:
        ...
        print(
            "Phenol to boronic ester strategy detected (target molecule contains boronic ester)"
        )
        return True
