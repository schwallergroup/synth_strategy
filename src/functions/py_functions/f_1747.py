    ...
    def dfs_traverse(node):
        ...
        if (
            ...
            if is_esterification or is_hydrolysis:
                print(
                    f"Found {'esterification' if is_esterification else 'hydrolysis'} reaction"
                )

                # For esterification: alcohol in reactants, ester in products
                # For hydrolysis: ester in reactants, alcohol in products
                alcohol_molecules = (
                    reactants_part.split(".")
                    if is_esterification
                    else products_part.split(".")
                )
                ester_molecules = (
                    products_part.split(".")
                    if is_esterification
                    else reactants_part.split(".")
                )
                    ...
                    if checker.check_fg("Tertiary alcohol", mol_smiles):
                        ...
                        if checker.check_ring("thiophene", mol_smiles):
                            ...
                            if thiophene_count >= 2:
                                print(
                                    "Found tertiary alcohol with at least 2 thiophene rings"
                                )
            ...
            for reactant in reactants_part.split("."):
                if checker.check_fg(
                    "Tertiary alcohol", reactant
                ) and checker.check_ring("thiophene", reactant):
                    thiophene_indices = checker.get_ring_atom_indices(
                        "thiophene", reactant
                    )
                    if len(thiophene_indices) >= 2:
                        print(
                            "Found tertiary alcohol with at least 2 thiophene rings in reactants"
                        )
                        ...
                        for product in products_part.split("."):
                            if checker.check_fg(
                                ...
                                product_thiophene_indices = (
                                    checker.get_ring_atom_indices("thiophene", product)
                                )
                                if len(product_thiophene_indices) >= 2:
                                    print(
                                        "Found ester with at least 2 thiophene rings in products"
                                    )
                                    found_bis_thiophene_ester = True
                                    return  # Early return once found
