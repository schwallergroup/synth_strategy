    ...
    def dfs_traverse(node, depth=0):
        ...
        if (
            ...
            reactant_has_aryl_halide = any(
                r is not None and r.HasSubstructMatch(aryl_halide_pattern)
                for r in reactants
            )
            ...
            if reactant_has_aryl_halide and product is not None:
                ...
                product_aryl_n_count = product.GetSubstructMatches(
                    aryl_n_pattern
                ).__len__()

                if product_aryl_n_count > reactant_aryl_n_count:
                    print(
                        f"Detected late-stage arylation via C-N bond formation at depth {depth}"
                    )
