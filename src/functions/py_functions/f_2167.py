    ...
    def dfs_traverse(node, depth=0):
        ...
        if (
            ...
            if is_snar_reaction:
                found_snar = True
                print(
                    f"Found nucleophilic aromatic substitution reaction at depth: {depth}"
                )
                return
                ...
                if checker.check_fg("Aromatic halide", reactant):
                    ...
                    for ring in heterocycle_rings:
                        if checker.check_ring(ring, reactant):
                            ...
                            print(
                                f"Found halogenated heterocycle ({ring}) in reactant: {reactant}"
                            )
                            break
            ...
            if (
                has_halogenated_heterocycle
                and has_nucleophile
                and product_has_c_hetero_bond
            ):
                # Heterocycles are inherently electron-deficient, so EWG is not strictly required
                found_snar = True
                print(
                    f"Found nucleophilic aromatic substitution pattern at depth: {depth}"
                )
