    ...
    def dfs_traverse(node, depth=0):
        ...
        if depth == 0 and node["type"] == "mol":
            ...
            if checker.check_fg("Nitrile", mol_smiles) and checker.check_ring(
                ...
                print(
                    f"Found cyano group in final product (not on pyrimidine): {mol_smiles}"
                )
                has_cyano_group = True
        ...
        if (
            ...
            if checker.check_ring("pyrimidine", product):
                ...
                pyrimidine_reactants = [
                    r for r in reactants if checker.check_ring("pyrimidine", r)
                ]
                ...
                if pyrimidine_reactants:
                    ...
                    is_snar = any(
                        checker.check_reaction(rxn, rsmi) for rxn in snar_reaction_types
                    )
                    ...
                    if is_snar:
                        print(
                            f"Found pyrimidine SNAr reaction at depth {depth}: {rsmi}"
                        )
