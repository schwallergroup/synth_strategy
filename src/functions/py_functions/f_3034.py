...
def main(route):
        ...
        if node["type"] == "reaction":
            try:
                ...
                if product_mol is None or any(mol is None for mol in reactant_mols):
                    print(
                        f"Warning: Could not parse some molecules in reaction: {rsmi}"
                    )
                    return
    ...
    valid_fg_mods = [
        (mod, d) for mod, d in fg_modifications if d < ring_formation_depth
    ]
    ...
    if strategy_detected and len(valid_fg_mods) >= 2:
        ...
        for i in range(len(depths) - 1):
            ...
            ):  # Allow some flexibility in "sequential" definition
                is_sequential = False
                break
    ...
    print(
        f"Heterocyclic ring formation followed by FG modifications: {strategy_detected}"
    )
    print(
        f"Ring formation depth: {ring_formation_depth if ring_formation_found else 'Not found'}"
    )
