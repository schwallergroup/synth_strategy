...
def main(route):
        ...
        if (
            ...
            if all(reactant_mols) and product_mol:
                ...
                reactants_with_ether = any(
                    mol.HasSubstructMatch(aromatic_ether_pattern)
                    for mol in reactant_mols
                )
                product_has_ether = product_mol.HasSubstructMatch(
                    aromatic_ether_pattern
                )
    ...
    if ether_formation_depth is not None and max_depth > 0:
        ...
        if (
            ether_formation_depth >= max_depth * 0.7
        ):  # Consider "early" as in the last 30% of steps
            print(
