...
def main(route):
            ...
            try:
                ...
                for reactant in reactants_smiles:
                    for ring_type in ring_types:
                        if checker.check_ring(ring_type, reactant):
                            reactant_rings[ring_type] = (
                                reactant_rings.get(ring_type, 0) + 1
                            )
                ...
                for ring_type in product_rings:
                    if ring_type not in reactant_rings:
                        print(
                            f"Ring disconnection detected: {ring_type} at depth {depth}"
                        )
                        if (
                            ring_disconnection_depth is None
                            or depth < ring_disconnection_depth
                        ):
    ...
    print(
        f"Late-stage ring disconnection detected: {result} (depth: {ring_disconnection_depth})"
    )
