...
def main(route):
    ...
    def dfs_traverse(node):
        ...
        if (
            ...
            benzyl_bromide_pattern = Chem.MolFromSmarts(
                "Br[CH2][c]1[cH][cH][cH][cH][cH]1"
            )
            benzyl_ether_pattern = Chem.MolFromSmarts(
                "[O][CH2][c]1[cH][cH][cH][cH][cH]1"
            )
            ...
            if product_mol and all(reactant_mols):
                ...
                has_benzyl_bromide = any(
                    m and m.HasSubstructMatch(benzyl_bromide_pattern)
                ...
                has_boc = any(
                    m and m.HasSubstructMatch(boc_group_pattern) for m in reactant_mols
                )
