    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "reaction":
            if "rsmi" in node.get("metadata", {}):
                ...
                n_alkylated_indole_pattern = Chem.MolFromSmarts(
                    "[n]([CH2]c)1c2ccccc2cc1"
                )
                ...
                if (
                    product_mol
                    and any(
                        m and m.HasSubstructMatch(indole_nh_pattern)
                        for m in reactant_mols
                    )
                    and any(
                        m and m.HasSubstructMatch(benzyl_halide_pattern)
                        for m in reactant_mols
                    )
                ):
