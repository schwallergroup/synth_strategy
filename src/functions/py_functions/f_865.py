                ...
                for ketone_atoms in ketone_indices:
                    ...
                    for bond in mol.GetBonds():
                        ...
                        if (
                            ...
                            print(
                                f"Found pyridone in final product: {final_product_smiles}"
                            )
    ...
    def track_heterocycles(node, depth=0):
        if (
            ...
            if heterocycle_tracking["pyridone"]["preserved"]:
                ...
                for r in reactants:
                    if checker.check_ring("pyridine", r) and checker.check_fg(
                        "Ketone", r
                    ):
                ...
                if checker.check_ring("pyridine", product) and checker.check_fg(
                    "Ketone", product
                ):
                    pyridine_indices = checker.get_ring_atom_indices(
                        "pyridine", product
    ...
    if mol:
        ...
        if not heterocycle_tracking["triazole"]["present_in_final"]:
            ...
            for pattern in triazole_patterns:
                if pattern and mol.HasSubstructMatch(pattern):
                    print(
                        f"Found triazole pattern in final product: {final_product_smiles}"
                    )
                    heterocycle_tracking["triazole"]["present_in_final"] = True
                    break
        ...
        if not heterocycle_tracking["pyridone"]["present_in_final"]:
            ...
            for pattern in pyridone_patterns:
                if pattern and mol.HasSubstructMatch(pattern):
                    print(
                        f"Found pyridone pattern in final product: {final_product_smiles}"
                    )
                    heterocycle_tracking["pyridone"]["present_in_final"] = True
                    break
