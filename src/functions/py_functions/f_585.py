        ...
        for i in range(len(path) - 1):
            if path[i]["type"] == "reaction" and path[i + 1]["type"] == "mol":
                ...
                if checker.check_fg("Tertiary amine", product) and any(
                    ...
                    print(
                        f"Found amine functionalization (methylation) at index {i}: {rsmi}"
                    )
                    amine_functionalization_indices.append(i)
                elif any(
                    checker.check_reaction(rxn, rsmi)
                    for rxn in amine_functionalization_reactions
                ):
                ...
                # Alternative check for nitro reduction
                    ...
                    print(
                        f"Found nitro reduction (alternative check) at index {i}: {rsmi}"
                    )
                    nitro_reduction_indices.append(i)
