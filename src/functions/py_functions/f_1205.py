...
def main(route):
    ...
    def check_amine_diversification(reaction_node):
        ...
        if "metadata" in reaction_node and "rsmi" in reaction_node["metadata"]:
            ...
            has_secondary_amine_product = checker.check_fg(
                "Secondary amine", product_part
            )
            has_tertiary_amine_product = checker.check_fg(
                "Tertiary amine", product_part
            )
    ...
    if not final_reactions or not found_late_amine:
        ...
        def find_last_reaction(
            node, current_depth=0, min_depth=float("inf"), last_reaction=None
        ):
            if (
                node["type"] == "reaction"
                and "metadata" in node
