...
def main(route):
        ...
        if node["type"] == "mol":
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if checker.check_reaction("Protection of carboxylic acid", rsmi):
                    if "OC(C)(C)C" in product and not any(
                        "OC(C)(C)C" in r for r in reactants
                    ):
                        has_tbutyl_protection = True
                        print(
                            f"Found t-butyl protection via reaction check at depth {depth}: {rsmi}"
                        )
                    if "COC(=O)" in product and not any(
                        "COC(=O)" in r for r in reactants
                    ):
                        has_methyl_protection = True
                        print(
    ...
    print(
        f"Orthogonal carboxylic acid protection strategy detected: {strategy_present}"
    )
