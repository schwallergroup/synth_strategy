...
def main(route):
            ...
            for rxn_type in reductive_amination_rxns:
                if checker.check_reaction(rxn_type, rsmi):
                    ...
                    if (
                        (
                            carbonyl_present
                            or any(checker.check_fg("Alcohol", r) for r in reactants)
                        )
                        and amine_present
                        and amine_product
                    ):
                        ...
                        print(
                            f"  Reductive amination detected in reaction {reaction_count}"
                        )
                        break
    ...
    if len(reaction_sequence) >= 2:
        for i in range(len(reaction_sequence) - 1):
            ...
            if not product_found:
                is_linear = False
                print(
                    f"Non-linear transition detected between reactions {i+1} and {i+2}"
                )
                break
    else:
    ...
    linear_with_transformations = (
        reaction_count >= 2 and transformations_count >= 1 and is_linear
    )
