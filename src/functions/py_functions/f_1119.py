...
def main(route):
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if not is_snar:
                    ...
                    if has_cf3_pyridine_with_leaving_group and has_nucleophile:
                        ...
                        ) and checker.check_fg("Trifluoro group", product_smiles):
                            is_snar = True
                ...
                if is_snar:
                    ...
                    for r in reactants:
                        if checker.check_fg(
                            "Trifluoro group", r
                        ) and checker.check_ring("pyridine", r):
                ...
                for rxn_type in ring_forming_reactions:
                    if checker.check_reaction(rxn_type, rsmi):
                        ...
                        if checker.check_ring(
                            ...
                            cyclization_reactions.append((depth, rsmi))
                            print(
                                f"Found cyclization reaction type: {rxn_type} at depth {depth}"
                            )
                            break
    ...
    print(
        f"Found {len(snar_reactions)} SNAr reactions at depths: {[r[0] for r in snar_reactions]}"
    )
    print(
