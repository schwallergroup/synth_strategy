...
def main(route):
        ...
        for reaction in reactions_by_depth[depth]:
            try:
                ...
                if not is_cross_coupling:
                    ...
                    has_halide = any(
                        ("Br" in r or "Cl" in r or "I" in r)
                        and not ("N" in r and "Cl" in r)
                        for r in reactants
                    )

                    if has_boronic and has_halide:
                        print(
                            f"Manually identified potential Suzuki coupling at depth {depth}"
                        )
                        is_cross_coupling = True
                ...
                if (
                    ...
                    or checker.check_reaction(
                        "Ullmann-Goldberg Substitution thiol", rsmi
                    )
                    or checker.check_reaction(
    ...
    print(
        f"Cross-coupling found: {found_cross_coupling} at depth {cross_coupling_depth}"
    )
    print(
    ...
    if found_cross_coupling and found_nucleophilic_substitution:
        ...
        if nucleophilic_subst_depth < cross_coupling_depth:
            print(
                "Correct sequence: cross-coupling followed by nucleophilic substitution"
            )
            correct_sequence = True
        else:
            print(
                "Incorrect sequence: nucleophilic substitution does not follow cross-coupling"
            )
