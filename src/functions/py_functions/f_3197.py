...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            if node["smiles"]:
                ...
                if has_heterocycle_struct(node["smiles"]) and has_fluorine(
                    node["smiles"]
                ):
                    print(
                        f"Found molecule with heterocycle and fluorine: {node['smiles']}"
                    )
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if not has_heterocycle_formation:
                    ...
                    if has_heterocycle_struct(product) and not all(
                        ...
                        has_heterocycle_formation = True
                        print(
                            f"Found general heterocycle formation reaction: {rxn_smiles}"
                        )

                    # Check if reaction modifies a heterocycle and involves fluorine
                    if has_heterocycle_struct(product) and any(
                        has_fluorine(r) for r in reactants
                    ):
                        print(
                            "Reaction modifies heterocycle with fluorinated building block"
                        )

                        # If the product has fluorine that wasn't in the heterocycle reactants
                        if has_fluorine(product) and not any(
                            has_heterocycle_struct(r) and has_fluorine(r)
                            for r in reactants
                        ):
                            has_heterocycle_formation = True
    ...
    print(
        f"  Fluorinated heterocycle in final product: {has_fluorinated_heterocycle_product}"
    )
