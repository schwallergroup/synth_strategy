...
def main(route):
    ...
    def is_heterocycle_formation(rsmi):
        ...
        if checker.check_reaction("imidazole", rsmi) or checker.check_reaction(
            "{imidazole}", rsmi
        ):
            return True
        ...
        if node["type"] == "reaction":
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                for rxn_type in convergent_reactions:
                    if checker.check_reaction(rxn_type, rsmi):
                        print(
                            f"Detected convergent reaction type: {rxn_type} in {rsmi}"
                        )
                        is_linear = False
                        return
                ...
                for reactant in reactants:
                    ...
                    if (
                        reactant_mol and reactant_mol.GetNumHeavyAtoms() > 12
                    ):  # Increased threshold
                ...
                if complex_reactant_count > 1:
                    ...
                    if not any(
                        ...
                        for rxn in linear_reaction_types
                    ):
                        print(
