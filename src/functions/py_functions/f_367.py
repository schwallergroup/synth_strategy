...
def main(route):
            ...
            try:
                ...
                if not pyrazolopyrimidine_formation:
                    ...
                    product_has_pyrimidine = checker.check_ring(
                        "pyrimidine", product_part
                    )

                    # Check if any reactant has both rings
                    reactants_have_both = any(
                        checker.check_ring("pyrazole", r)
                        and checker.check_ring("pyrimidine", r)
                        for r in reactants
                        if r
                    )
                    ...
                    if (
                        ...
                        if product_mol:
                            ...
                            for atom_idx in range(product_mol.GetNumAtoms()):
                                ...
                                if 5 in ring_sizes and 6 in ring_sizes:
                                    print(
                                        f"Detected pyrazolopyrimidine formation at depth {depth}"
                                    )
    ...
    strategy_present = (
        pyrazole_formation
        and pyrazolopyrimidine_formation
        and final_product_has_pyrazolopyrimidine
    )

    print(f"Pyrazole formation: {pyrazole_formation}")
    print(f"Pyrazolopyrimidine formation: {pyrazolopyrimidine_formation}")
    print(
        f"Final product has pyrazolopyrimidine: {final_product_has_pyrazolopyrimidine}"
    )
