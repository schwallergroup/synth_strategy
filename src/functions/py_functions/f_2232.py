...
def main(route):
        ...
        if node["type"] == "reaction":
            if "metadata" in node and "rsmi" in node["metadata"]:
                try:
                    ...
                    ) and checker.check_fg("Tertiary amide", product):
                        functional_group_sequence.append(
                            ("carboxylic_acid_to_weinreb", depth)
                        )
    ...
    has_progression = any(
        is_subsequence(prog, transformations) for prog in expected_progressions
    )
