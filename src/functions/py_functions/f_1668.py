...
def main(route):
    ...
    print(
        f"Heterocycle formation: {heterocycle_formation} at depth {heterocycle_depth}"
    )
    print(
        f"Halogen displacements: {halogen_displacements} at depths {halogen_displacement_depths}"
    )
    ...
    if heterocycle_formation and halogen_displacements >= 1:
        ...
        if all(heterocycle_depth > d for d in halogen_displacement_depths):
            print(
                "Heterocycle formation occurs earlier in synthesis than halogen displacements"
            )
            ...
            if halogen_displacements >= 2:
                ...
                for i in range(len(sorted_depths) - 1):
                    if (
                        abs(sorted_depths[i] - sorted_depths[i + 1]) <= 3
