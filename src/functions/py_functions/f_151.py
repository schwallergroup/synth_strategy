...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        if node["type"] == "mol":
            ...
            if checker.check_reaction(
                "Reduction of aldehydes and ketones to alcohols", rxn_smiles
            ):
    ...
    aldehyde_depths = (
        sorted(functional_groups["aldehyde"].keys())
        if functional_groups["aldehyde"]
        else []
    )
    alcohol_depths = (
        sorted(functional_groups["alcohol"].keys())
        if functional_groups["alcohol"]
        else []
    )
    ketone_depths = (
        sorted(functional_groups["ketone"].keys())
        if functional_groups["ketone"]
        else []
    )
    ...
    ester_depths = (
        sorted(functional_groups["ester"].keys()) if functional_groups["ester"] else []
    )
