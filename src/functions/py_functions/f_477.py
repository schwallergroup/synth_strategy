...
def main(route):
    ...
    def dfs_traverse(node):
        if node["type"] == "reaction":
            ...
            if product_mol and reactant_mols:
                ...
                if any(
                    mol.HasSubstructMatch(Chem.MolFromSmarts("c[Br]"))
                ...
                if any(
                    mol.HasSubstructMatch(Chem.MolFromSmarts("cC#N"))
                    for mol in reactant_mols
                ...
                if any(
                    mol.HasSubstructMatch(Chem.MolFromSmarts("cC(=O)O"))
                    for mol in reactant_mols
    ...
    has_sequence = all(
        t in transformations for t in ["Br_to_CN", "CN_to_COOH", "COOH_to_amide"]
    )

    print(
        f"Sequential functional group transformation strategy detected: {has_sequence}"
    )
