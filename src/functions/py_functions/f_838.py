        ...
        if node["type"] == "reaction":
            try:
                ...
                if (
                    ...
                    or checker.check_reaction(
                        "Ester with primary amine to amide", reaction_smiles
                    )
                    or checker.check_reaction(
                        "Ester with secondary amine to amide", reaction_smiles
                    )
                    or checker.check_reaction(
                        "Acylation of primary amines", reaction_smiles
                    )
                    or checker.check_reaction(
                        "Acylation of secondary amines", reaction_smiles
                    )
                    or checker.check_reaction(
                        "Schotten-Baumann to ester", reaction_smiles
                    )
                    or checker.check_reaction(
                        "{Schotten-Baumann_amide}", reaction_smiles
                    )
                ):
                ...
                if checker.check_reaction(
                    "Reduction of nitro groups to amines", reaction_smiles
                ):
                ...
                if (
                    checker.check_reaction(
                        "Acyl chlorides from alcohols", reaction_smiles
                    )
                    or checker.check_reaction(
                        "Acyl bromides from alcohols", reaction_smiles
                    )
                    or checker.check_reaction(
                        "Acyl iodides from alcohols", reaction_smiles
                    )
                ):
                ...
                if checker.check_fg("Acyl halide", product_smiles):
                    for reactant in reactants_smiles:
                        if checker.check_fg("Carboxylic acid", reactant):
                            print(
                                f"Found acid activation (COOH → COX) at depth {depth}"
                            )
                            has_acid_activation = True
                            break
                ...
                for reactant in reactants_smiles:
                    if checker.check_fg("Carboxylic acid", reactant):
                        ...
                        ) or checker.check_fg("Anhydride", product_smiles):
                            print(
                                f"Found acid activation (other method) at depth {depth}"
                            )
                            has_acid_activation = True
                            break
