...
def main(route):
    ...
    cycle_count = sum(
        1
        for pg, cycle in complete_cycles.items()
        if cycle["protection"] and cycle["deprotection"]
    )

    # Strategy requires at least 2 complete protection-deprotection cycles
    result = cycle_count >= 2
    print(
        f"Multiple amine protection-deprotection cycles: {result} (found {cycle_count} cycles)"
    )
    return result
