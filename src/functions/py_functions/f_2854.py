...
def main(route):
            ...
            if "rsmi" in node.get("metadata", {}):
                try:
                    ...
                    if not is_amide_coupling:
                        ...
                        primary_amide_in_product = checker.check_fg(
                            "Primary amide", product
                        )
                        secondary_amide_in_product = checker.check_fg(
                            "Secondary amide", product
                        )
                        tertiary_amide_in_product = checker.check_fg(
    ...
    if amide_coupling_depth is not None and amide_coupling_depth <= 1:
        print(
            f"Late-stage amide coupling strategy detected at depth {amide_coupling_depth}"
        )
        return True
