...
def main(route):
            ...
            if depth == 1:
                ...
                if checker.check_ring("benzimidazole", product_smiles):
                    ...
                    is_benzimidazole_formation = any(
                        ...
                        for rxn in benzimidazole_reactions
                    )
                ...
                for mol, info in molecule_track.items():
                    if (
                        info["depth"] == depth - 1
                        and product_smiles in info["reactants"]
                    ):
                    ...
                    if depth == 3:
                        ...
                        if not snar_reaction:
                            ...
                            if (halide_reactant or fluoro_reactant) and amine_reactant:
                                print(
                                    "Detected SNAr reaction at depth 5 (using functional groups)"
                                )
                                snar_reaction = True
                        ...
                        for rxn_type in suzuki_reactions:
                            if checker.check_reaction(rxn_type, rsmi):
                                print(
                                    f"Detected Suzuki coupling ({rxn_type}) at depth {depth}"
                                )
                        ...
                        if not suzuki_coupling:
                            ...
                            boronic_ester = any(
                                checker.check_fg("Boronic ester", r)
                                for r in reactants_smiles
                            )
                            aryl_halide = any(
                                checker.check_fg("Aromatic halide", r)
    ...
    strategy_present = (
        ...
        and nitro_reduction
        and (
            suzuki_coupling or snar_reaction
        )  # Either Suzuki or SNAr should be present
    )
