...
def main(route):
    ...
    def get_reaction_pattern(rsmi):
        ...
        if checker.check_reaction(
            "Preparation of boronic acids", rsmi
            ...
            if len(paths_with_depths) >= 2:
                ...
                for i in range(len(paths)):
                    for j in range(i + 1, len(paths)):
                        ...
                        if divergence_point < min_path_len or len(paths[i]) != len(
                            paths[j]
                        ):
                            print(
    ...
    if not reaction_patterns and not coupling_reactions:
        print(
            "No relevant reactions found in the route. Check if the route contains reactions."
        )
        return False
