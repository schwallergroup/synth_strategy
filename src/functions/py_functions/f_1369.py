    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "reaction":
            ...
            reactant_has_hydroxypyrimidine = any(
                r is not None and r.HasSubstructMatch(hydroxypyrimidine)
                for r in reactants
            )
            product_has_chloropyrimidine = (
                product is not None and product.HasSubstructMatch(chloropyrimidine)
            )
            ...
            reactant_has_chloropyrimidine = any(
                r is not None and r.HasSubstructMatch(chloropyrimidine)
                for r in reactants
            )
            product_has_aminopyrimidine = (
                product is not None and product.HasSubstructMatch(aminopyrimidine)
            )
