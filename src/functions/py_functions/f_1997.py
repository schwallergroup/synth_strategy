...
def main(route):
            ...
            if rsmi:
                ...
                if current_depth >= 5:  # Early stage reaction based on stdout
                    ...
                    if "c1c(=O)[nH]c2ccccc12" in product or checker.check_fg(
                        ...
                        if not reactant_has_quinolinone:
                            print(
                                f"Detected quinolinone formation at depth {current_depth}"
                            )
                ...
                if 3 <= current_depth <= 7:  # Expanded middle stage range
                    ...
                    if (
                        ...
                        has_benzyl_halide = any(
                            checker.check_fg("Primary halide", r) and "CH2" in r
                            for r in reactants
                        )
                        has_amine = any(
                ...
                if current_depth <= 2:  # Late stage reaction based on stdout
                    ...
                    if piperazine_reactant:
                        print(
                            f"Detected piperazine as reactant at depth {current_depth}"
                        )
    ...
    return (
        has_quinolinone_formation
        and has_n_alkylation
        and has_late_stage_snar_with_piperazine
    )
