...
def main(route):
        ...
        if node["type"] == "mol":
            ...
            if depth > 0 and not node.get("in_stock", False):
                if not has_halogen(node["smiles"]):
                    ...
                    print(
                        f"No halogen found in intermediate at depth {depth}: {node['smiles']}"
                    )
                    ...
                    if product_mol:
                        for bond in product_mol.GetBonds():
                            if (
                                ...
                                print(
                                    f"Found potential Wittig-like olefination at depth {depth}"
                                )
                                wittig_found = True
                                wittig_depth = depth
                                break
    ...
    result = (
        ...
        and (  # Ring formation is essential
            nitrile_reduction_found or formylation_found
        )
        and (  # Either nitrile reduction or formylation
            wittig_found or late_stage_wittig
        )
    ...
    if result:
        print(
            "Detected complete heterocycle construction strategy with late-stage olefination"
        )
