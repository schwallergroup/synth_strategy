    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "reaction" and "rsmi" in node.get("metadata", {}):
            ...
            if product_mol and product_mol.HasSubstructMatch(sulfonamide_pattern):
                has_amine = any(
                    m and m.HasSubstructMatch(amine_pattern) for m in reactant_mols
                )
                has_sulfonyl_chloride = any(
                    m and m.HasSubstructMatch(sulfonyl_chloride_pattern)
                ...
                if has_amine and has_sulfonyl_chloride:
                    ...
                    print(
                        "Found sulfonamide formation from amine and sulfonyl chloride"
                    )
