...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if (
        ...
        ):  # Consider depth 0-2 as late-stage
            if "rsmi" in node.get("metadata", {}):
                ...
                if is_oxidation:
                    print(
                        f"Found potential oxidation reaction at depth {depth}: {rsmi}"
                    )

                    # Check if product has nitrogen-containing functional groups
                    product_n_fgs = [
                        fg for fg in n_containing_fgs if checker.check_fg(fg, product)
                    ]
                    ...
                    if product_n_fgs or product_n_rings:
                        ...
                        for reactant in reactants:
                            ...
                            if set(product_n_fgs) != set(reactant_n_fgs):
                                print(
                                    f"Found N-oxidation (FG change) at depth {depth}: {rsmi}"
                                )
                        ...
                        for reactant in reactants:
                            ...
                            if set(product_n_rings) != set(reactant_n_rings):
                                print(
                                    f"Found N-oxidation (ring change) at depth {depth}: {rsmi}"
                                )
                                n_oxidation_at_low_depth = True
                                break
