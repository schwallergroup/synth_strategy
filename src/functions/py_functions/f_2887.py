...
def main(route):
    ...
    weinreb_to_ketone_reactions = (
        []
    )  # Store reactions converting Weinreb amides to ketones
    ...
    def dfs_traverse(node, depth=0):
        if node["type"] == "mol":
            ...
            if weinreb_reactant and checker.check_fg("Ketone", product):
                print(
                    f"Found conversion of Weinreb amide to ketone at depth {depth}: {rsmi}"
                )
            ...
            if checker.check_reaction("Ketone from Weinreb amide", rsmi):
                print(
                    f"Found specific Weinreb amide to ketone reaction at depth {depth}: {rsmi}"
                )
                weinreb_to_ketone_reactions.append(rsmi)
            ...
            if any(checker.check_fg("Weinreb amide", r) for r in reactants) and any(
                ...
                if checker.check_fg("Ketone", product):
                    print(
                        f"Found Grignard reaction with Weinreb amide at depth {depth}: {rsmi}"
                    )
                    weinreb_to_ketone_reactions.append(rsmi)
