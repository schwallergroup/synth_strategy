            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                try:
                    ...
                    reactants_have_nitro = any(
                        r.HasSubstructMatch(nitro_pattern)
                        for r in reactants
                        if r is not None
                    )
                    product_has_amine = (
                        product is not None and product.HasSubstructMatch(amine_pattern)
                    )
                    ...
                    reactants_have_amine = any(
                        r.HasSubstructMatch(amine_pattern)
                        for r in reactants
                        if r is not None
                    )
                    product_has_tertiary_amine = (
                        product is not None
                        and product.HasSubstructMatch(tertiary_amine_pattern)
                    )
