...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        if (
            ...
            if checker.check_reaction(
                ...
                and not any(
                    checker.check_fg("Phenol", reactant) for reactant in reactants
                )
            ):
    ...
    mom_protection_cycle = (
        len(mom_protected_molecules) > 0 and len(mom_deprotected_molecules) > 0
    )
    boc_protection_cycle = (
        len(boc_protected_molecules) > 0 and len(boc_deprotected_molecules) > 0
    )
