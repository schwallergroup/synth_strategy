    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "mol" and "smiles" in node:
            ...
            if checker.check_ring("pyrazole", mol_smiles):
                ...
                print(
                    f"Found {len(pyrazole_indices)} pyrazole rings in molecule: {mol_smiles}"
                )
                ...
                if "cnn1-c1" in mol_smiles or "nn(C)c" in mol_smiles:
                    ...
                    if pyrazole_count >= 2:
                        print(
                            f"Found approximately {pyrazole_count} pyrazole-like structures"
                        )
                    else:
                        all_steps_have_pyrazoles = False
                        print(
                            f"Insufficient pyrazole-like structures: {pyrazole_count}"
                        )
                else:
                    all_steps_have_pyrazoles = False
