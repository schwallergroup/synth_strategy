...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        print(
            f"Visiting node at depth {depth}: {node.get('type')} - {node.get('smiles', '')}"
        )
            ...
            if node.get("metadata", {}).get("rsmi"):
                ...
                for reaction_type in cross_coupling_reactions:
                    if checker.check_reaction(reaction_type, rsmi):
                        ...
                        print(
                            f"Detected cross-coupling reaction: {reaction_type} in {rsmi}"
                        )
                ...
                if not is_cross_coupling:
                    ...
                    if (
                        has_pd_catalyst
                        and (has_boronic or has_metal_reactant)
                        and has_aryl_halide
                    ):
                        cross_coupling_count += 1
                        print(
                            f"Detected cross-coupling reaction by functional groups in {rsmi}"
                        )
