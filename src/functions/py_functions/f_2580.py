...
def main(route):
    ...
    def has_two_aromatic_fragments(mol_smiles, piperazine_indices=None):
        ...
        try:
            ...
            print(
                f"Error processing piperazine indices: {piperazine_indices}, Error: {e}"
            )
            ...
            while queue:
                ...
                for neighbor in atom.GetNeighbors():
                    ...
                    if (
                        ...
                        and neighbor_idx not in piperazine_atoms
                    ):
        ...
        if node["type"] == "mol":
            ...
            if checker.check_ring("piperazine", node["smiles"]):
                ...
                if node == route:
                    ...
                    if has_two_aromatic_fragments(node["smiles"]):
                        ...
                        print(
                            "Piperazine connects two aromatic fragments in final product"
                        )
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                if product_has_piperazine:
                    ...
                    if len(reactants) >= 2 and (
                        len(reactants_with_piperazine) < len(reactants)
                    ):
                        # Either piperazine is newly formed or it's connecting to a new fragment
                        print(
                            f"Reaction potentially forms or modifies piperazine: {rsmi}"
                        )
    ...
    result = (
        piperazine_found and final_product_has_piperazine and connects_two_fragments
    )
