            ...
            if mol:
                ...
                for ring_name in aromatic_rings:
                    if checker.check_ring(ring_name, mol_smiles):
                        has_aromatic = True
                        print(
                            f"Found aromatic ring {ring_name} in molecule: {mol_smiles}"
                        )
                        break
                ...
                if has_aromatic:
                    ...
                    pattern4 = Chem.MolFromSmarts(
                        "a~[#6]~[#6]~[#6]~[#6]~a"
                    )  # 4-atom chain
                    pattern5 = Chem.MolFromSmarts(
                        "a~[#6]~[#6]~[#6]~[#6]~[#6]~a"
                    )  # 5-atom chain
                    ...
                    if product_has_aromatic:
                        ...
                        if product_has_chain:
                                ...
                                if r_mol:
                                    ...
                                    if reactant_has_aromatic:
                                        ...
                                        if (
                                            ...
                                            print(
                                                f"Reactant already has long chain: {reactant}"
                                            )
                                            break
                            ...
                            if not reactant_has_chain:
                                long_chain_connection = True
                                print(
                                    f"Found reaction creating long chain connection: {rsmi}"
                                )
                                ...
                                if checker.check_reaction(
                                    ...
                                    print(
                                        "Detected Suzuki coupling creating chain connection"
                                    )
                                elif checker.check_reaction(
                                    "Heck terminal vinyl", rsmi
                                ):
                                    print(
                                        "Detected Heck reaction creating chain connection"
                                    )
                                elif checker.check_reaction(
                                    "Sonogashira alkyne_aryl halide", rsmi
                                ):
                                    print(
                                        "Detected Sonogashira coupling creating chain connection"
                                    )
