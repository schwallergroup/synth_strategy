            ...
            try:
                ...
                for reaction_type in cyclization_reaction_types:
                    if checker.check_reaction(reaction_type, rsmi):
                        print(
                            f"Cyclization reaction detected: {reaction_type} at depth {depth}"
                        )
                ...
                if all(reactant_mols) and product_mol:
                    ...
                    if product_ring_count > max(reactant_ring_counts):
                        ...
                        is_cyclization_reaction = (
                            True  # Set to true when ring count increases
                        )

                        # Check if specific ring types are formed in the product but not in reactants
                        for ring_type in ring_types:
                            product_has_ring = checker.check_ring(
                            ...
                            if product_has_ring and not reactants_have_ring:
                                print(
                                    f"New ring type formed: {ring_type} at depth {depth}"
                                )
                                break
