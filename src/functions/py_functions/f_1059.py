...
def main(route):
        ...
        if node["type"] == "mol":
            ...
            for heterocycle in heterocycle_types:
                if checker.check_ring(heterocycle, mol_smiles):
                    ...
                    print(
                        f"Found heterocycle {heterocycle} at depth {depth}: {mol_smiles}"
                    )
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                if len(reactants) > 1:
                    convergent_steps += 1
                    print(
                        f"Found convergent step with {len(reactants)} reactants: {rsmi}"
                    )
                        ...
                        if checker.check_ring(product_heterocycle_type, reactant):
                            ...
                            for rxn_type in elaboration_reaction_types:
                                if checker.check_reaction(rxn_type, rsmi):
                                    elaboration_steps += 1
                                    print(
                                        f"Found elaboration step: {rxn_type} at depth {depth}"
                                    )
                                    break
    ...
    elaboration_percentage = (
        elaboration_steps / reaction_count if reaction_count > 0 else 0
    )
