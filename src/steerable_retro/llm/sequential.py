    ...
    async def update_usage(self, bytes_used):
        async with self.lock:
            ...
            if self.current_bytes >= 0.9 * self.max_bytes_per_hour:
                logger.warning(f"Approaching rate limit: {self.current_bytes}/{self.max_bytes_per_hour} bytes used")
            
            # Check if we've exceeded the limit
            if self.current_bytes >= self.max_bytes_per_hour:
                self.is_rate_limited = True
                logger.error(f"Rate limit exceeded: {self.current_bytes} bytes. Cooling down for {self.cooldown_minutes} minutes.")
                return True
    ...
    async def reset_after_cooldown(self):
        async with self.lock:
            if self.is_rate_limited:
                logger.info(f"Starting cooldown for {self.cooldown_minutes} minutes")
...
async def exponential_backoff_retry(func, *args, max_retries=5, initial_delay=1, **kwargs):
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "rate_limit_error" in str(e) or "RateLimitError" in str(e.__class__):
                logger.error(f"Rate limit error detected on attempt {attempt+1}: {str(e)}")
                
                # Trigger rate limit handling
                await rate_tracker.update_usage(rate_tracker.max_bytes_per_hour)  # Force rate limit mode
            ...
            logger.warning(f"Attempt {attempt+1} failed with error: {str(e)}. Retrying in {retry_delay:.2f} seconds...")
...
class LM(BaseModel):
    ...
    async def run(self, tree: ReactionTree, query: str):
        ...
        if self.model == "random":
            ...
            response = await exponential_backoff_retry(self._run_llm, rxn_msgs, query)
        return response
    ...
    async def iterate(
        ...
        while rewriting_count < rewriting_limit and not passed:
            rewriting_count += 1
            inputs = importlib.import_module("steerable_retro.llm.prompts.rewriting_prep").rewriting_prep
                    ...
                    messages=[
                        {
                            "role": "user",
                            "content": [
                                {"type": "text","text": rdkit_docs, "cache_control": {"type": "ephemeral"}},
                                {"type": "text", "text": dictionary_info, "cache_control": {"type": "ephemeral"}},
                                # {"type": "text", "text": synthetic_strategy, "cache_control": {"type": "ephemeral"}},
                                {"type": "text", "text": rewriting_prompt, "cache_control": {"type": "ephemeral"}},
    ...
    async def run_single_route(self, d, task, idx):
        ...
        try:
            ...
            module = importlib.import_module("steerable_retro.llm.prompts.code_rewriting_cod")
            ...
            if 'output' in locals():
                return output
            return {idx: {"strategy": "", "code_blocks": [], "passed": [], "stdout": [], "errors": []}}

    async def run_single_task(self, task, data, nroutes=10):
        result = await asyncio.gather(
            *[self.run_single_route(task, d, idx) for idx, d in enumerate(data[:nroutes])]
        )
        return result
...
async def process_file(file_path, lm, n_samples=125, max_concurrent=5, start_idx=0):
    ...
    async def process_route(d, idx):
        async with semaphore:
            ...
            if await rate_tracker.check_rate_limit():
                logger.warning(f"Rate limit reached before processing route {idx}, waiting for cooldown")
                await rate_tracker.reset_after_cooldown()
    ...
    for batch_start in range(0, len(sampled_data), batch_size):
        ...
        print(f"Processing batch {batch_start//batch_size + 1}, routes {batch_start} to {batch_end-1}")
        
        # Create tasks for batch processing
        tasks = [process_route(d, idx) for idx, d in zip(batch_indices, batch_data)]
