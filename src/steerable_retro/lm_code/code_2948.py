    ...
    if not has_benzofuran_in_product:
        ...
        if mol:
            ...
            if benzofuran_pattern:
                has_benzofuran_in_product = len(mol.GetSubstructMatches(benzofuran_pattern)) > 0
        ...
        if not has_thiophene_in_product:
            ...
            if mol:
                ...
                if thiophene_pattern:
                    has_thiophene_in_product = len(mol.GetSubstructMatches(thiophene_pattern)) > 0
            ...
            try:
                ...
                if has_thiophene_in_product:
                    ...
                    if not product_has_thiophene:
                        # Check for thiazole
                        product_has_thiophene = checker.check_ring("thiazole", product_smiles)
                        if not product_has_thiophene:
                            # Try manual detection
                            mol = Chem.MolFromSmiles(product_smiles)
                            if mol:
