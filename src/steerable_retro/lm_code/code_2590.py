...
def main(route):
    ...
    final_has_benzothiophene = checker.check_ring("benzothiophene", final_product_smiles) or (
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            ...
            if checker.check_ring("dioxolane", mol_smiles) and checker.check_ring(
                ...
                print(f"Found benzodioxole-like structure at depth {depth}: {mol_smiles}")
            # If dioxolane check fails, look for the OCO pattern in the SMILES which indicates 1,3-benzodioxole
            ...
            if checker.check_reaction("Formation of NOS Heterocycles", rxn_smiles):
                ...
                if checker.check_ring("isoxazole", product) and not any(
                    ...
                    print(f"Found isoxazole formation reaction at depth {depth}: {rxn_smiles}")

            # Check for reactions that form benzothiophene
            if checker.check_reaction("benzothiophene", rxn_smiles) or checker.check_reaction(
                "thiophene", rxn_smiles
            ):
                # Check if this reaction produces a benzothiophene that wasn't in the reactants
                has_benzothiophene_product = checker.check_ring("benzothiophene", product) or (
                ...
                has_benzothiophene_reactants = any(
                    ...
                    or (checker.check_ring("benzene", r) and checker.check_ring("thiophene", r))
                    for r in reactants
                )

                if has_benzothiophene_product and not has_benzothiophene_reactants:
                    benzothiophene_formation = True
                    print(f"Found benzothiophene formation reaction at depth {depth}: {rxn_smiles}")
            ...
            if checker.check_reaction(
                ...
                has_benzodioxole_reactants = any(
                    (checker.check_ring("dioxolane", r) and checker.check_ring("benzene", r))
                ...
                if has_benzodioxole_product and not has_benzodioxole_reactants:
                    ...
                    print(f"Found benzodioxole formation reaction at depth {depth}: {rxn_smiles}")
