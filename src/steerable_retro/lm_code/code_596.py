        ...
        if node["type"] == "mol":
            ...
            try:
                ...
                if checker.check_ring("oxirane", product) and not any(
                    ...
                    if (
                        checker.check_reaction("Williamson Ether Synthesis (intra to epoxy)", rsmi)
                ...
                if any(checker.check_ring("oxirane", r) for r in reactants) and any(
                    ...
                    if not checker.check_ring("oxirane", product) and not checker.check_fg(
                        "Phenol", product
                    ):
                        if (
                ...
                if any(
                    ...
                    if any(checker.check_fg("Phenol", r) for r in reactants) or checker.check_fg(
                        "Ether", product
                    ):
                        phenol_alkylation_reactions[rsmi] = depth
                        print(f"Epoxide ring opening with phenol detected at depth {depth}: {rsmi}")
    ...
    if epoxide_molecules:
        ...
        if len(epoxide_molecules) >= 2:
            ...
            # If we don't have explicit reactions, infer from molecule patterns
                ...
                print("No explicit reactions detected, inferring from molecule patterns")

                # Sort epoxide molecules by depth
                sorted_epoxide_mols = sorted(epoxide_molecules.items(), key=lambda x: x[1])
                ...
                for later_smiles, later_depth in sorted_epoxide_mols[1:]:
                    if checker.check_fg("Phenol", later_smiles) and later_depth > earliest_depth:
                ...
                if len(epoxide_molecules) == 2 and (epoxide_depths[1] - epoxide_depths[0] >= 2):
                    # The deeper epoxide is likely a reagent for alkylation
                    deeper_epoxide = [
                        smiles for smiles, d in epoxide_molecules.items() if d == epoxide_depths[1]
                    ][0]
                    shallower_epoxide = [
                        smiles for smiles, d in epoxide_molecules.items() if d == epoxide_depths[0]
                    ][0]
                    ...
                    if checker.check_fg("Ether", shallower_epoxide):
                        ...
                        print(f"Shallow epoxide at depth {epoxide_depths[0]}: {shallower_epoxide}")
                        print(f"Deep epoxide at depth {epoxide_depths[1]}: {deeper_epoxide}")
                        return True
