    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "reaction":
            ...
            has_alkyl_bromide = any(
                r is not None and r.HasSubstructMatch(alkyl_bromide_pattern) for r in reactants
            )

            # Check for amine in reactants
            amine_pattern = Chem.MolFromSmarts("[#7;H1,H2]")
            has_amine = any(r is not None and r.HasSubstructMatch(amine_pattern) for r in reactants)
