...
def main(route):
        ...
        if node["type"] == "reaction":
            ...
            if node.get("metadata", {}).get("depth", 0) <= 1:
                try:
                    ...
                    has_hydrazine = any(
                        checker.check_fg("Hydrazine", reactant) for reactant in reactants
                    )
                    ...
                    if (
                        checker.check_reaction("Nitrile and hydrogen peroxide to amide", rsmi)
                        and has_hydrazine
                    ):
                        is_hydrazide_formation = True
    ...
    print(f"Late-stage hydrazide formation strategy detected: {has_late_stage_hydrazide}")
