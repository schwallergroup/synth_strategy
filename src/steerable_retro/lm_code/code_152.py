...
def main(route, min_count=3):
    ...
    def has_new_cc_bond(reactants_smiles, product_smiles, rsmi):
        ...
        try:
            ...
            reactants_mols = [Chem.MolFromSmiles(r) for r in reactants_smiles if r.strip()]
                ...
                if begin_atom.GetSymbol() == "C" and end_atom.GetSymbol() == "C":
                    ...
                    if begin_map > 0 and end_map > 0:
                        ...
                        for reactant_mol in reactants_mols:
                            ...
                            if begin_map in reactant_map_to_idx and end_map in reactant_map_to_idx:
                                begin_idx = reactant_map_to_idx[begin_map]
                                end_idx = reactant_map_to_idx[end_map]

                                # Check if they're bonded in the reactant
                                bond = reactant_mol.GetBondBetweenAtoms(begin_idx, end_idx)
                                if bond is not None:
            ...
            if product_mol:
                for atom in product_mol.GetAtoms():
                    if atom.GetSymbol() == "C":
                        ...
                        if any(
                            n.GetSymbol() == "O" and n.GetTotalNumHs() == 0 and n.GetDegree() == 1
                            for n in neighbors
                        ):
                ...
                if len(parts) >= 3:
                    ...
                    for reaction_type in cc_bond_forming_reactions:
                        if checker.check_reaction(reaction_type, rsmi):
                            print(f"  Matched known C-C bond forming reaction: {reaction_type}")
                            reaction_match = True
                            break
                    ...
                    if reaction_match or structure_match:
                        cc_bond_count += 1
                        print(f"  C-C bond formation detected! Current count: {cc_bond_count}")
