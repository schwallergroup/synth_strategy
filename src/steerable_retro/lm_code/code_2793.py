...
def main(route):
        ...
        if node["type"] == "reaction":
            ...
            try:
                ...
                if reactant_groups and product_groups:
                    transformation = f"{'+'.join(reactant_groups)} → {'+'.join(product_groups)}"
    ...
    for i in range(len(transformations) - 1):
        ...
        if "alkene" in current and "aldehyde" in next_trans and "alkene" not in next_trans:
            alkene_to_aldehyde = True

        if "aldehyde" in current and "alkene" in next_trans and "aldehyde" not in next_trans:
