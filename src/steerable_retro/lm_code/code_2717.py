...
def main(route):
    ...
    if final_product:
        final_product_has_halogens = len(final_product.GetSubstructMatches(halogen_query)) > 0
            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                for reaction_type in halogenation_reactions:
                    if checker.check_reaction(reaction_type, rsmi):
                        print(f"Halogenation reaction detected at depth {depth}: {reaction_type}")
                ...
                if not halogen_introduced:
                    ...
                    if all(reactant_mols) and product_mol:
                        ...
                        reactant_halogens = sum(
                            len(mol.GetSubstructMatches(halogen_query)) for mol in reactant_mols
                        )
                        product_halogens = len(product_mol.GetSubstructMatches(halogen_query))
                        ...
                        if not halogen_introduced:
                            for fg in halogen_fgs:
                                if checker.check_fg(fg, product_part):
                                    ...
                                    if not any(
                                        checker.check_fg(fg, r) for r in reactants_part.split(".")
                                    ):
                                        print(
