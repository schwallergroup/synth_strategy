            ...
            if "rsmi" in node.get("metadata", {}):
                ...
                try:
                    ...
                    if product and len(reactants) >= 2:
                        ...
                        has_oh = any(r and r.HasSubstructMatch(oh_pattern) for r in reactants if r)
                        has_br = any(r and r.HasSubstructMatch(br_pattern) for r in reactants if r)

                        # Check if product has new ether linkage
                        ether_pattern = Chem.MolFromSmarts("[#6]-[#8]-[#6]")
                        if has_oh and has_br and product.HasSubstructMatch(ether_pattern):
