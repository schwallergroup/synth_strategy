...
def main(route):
    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "reaction":
            ...
            has_amine = any(
                Chem.MolFromSmiles(r)
                and Chem.MolFromSmiles(r).HasSubstructMatch(Chem.MolFromSmarts("[NH2][c]"))
                for r in reactants
            )
            has_sulfonamide_product = Chem.MolFromSmiles(product) and Chem.MolFromSmiles(
                product
            ).HasSubstructMatch(Chem.MolFromSmarts("[NH][S](=[O])(=[O])[c]"))
