            ...
            if depth == 0:
                ...
                if mol:
                    ...
                    if mol.HasSubstructMatch(hydroxamic_pattern):
                        print(f"Hydroxamic acid detected in final product using SMARTS pattern")
                        hydroxamic_acid_present = True
                    # Alternative check using SMILES substring
                    elif "C(=O)NO" in node["smiles"]:
                        print(f"Hydroxamic acid detected in final product using SMILES substring")
                        hydroxamic_acid_present = True
                    # Check for combination of amide and hydroxyl
                    elif (
                        checker.check_fg("Primary amide", node["smiles"]) and "OH" in node["smiles"]
                    ):
