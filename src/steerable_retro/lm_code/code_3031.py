...
def main(route):
    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "mol":
            ...
            if checker.check_ring("piperazine", mol_smiles) and checker.check_fg("Boc", mol_smiles):
                ...
                if mol:
                    ...
                    for ring_atoms in piperazine_indices:
                        ...
                        if isinstance(ring_atoms[0], tuple):
                            ring_atoms = [atom for sublist in ring_atoms for atom in sublist]
                        else:
                            ring_atoms = ring_atoms

                        # Find nitrogen atoms in the piperazine ring
                        piperazine_nitrogens = [
                            idx for idx in ring_atoms if mol.GetAtomWithIdx(idx).GetAtomicNum() == 7
                        ]

                        for boc_atoms in boc_indices:
                            # Flatten the tuple of tuples if needed
                            if isinstance(boc_atoms[0], tuple):
                                boc_atoms = [atom for sublist in boc_atoms for atom in sublist]
                            else:
                                boc_atoms = boc_atoms
                            ...
                            for n_idx in piperazine_nitrogens:
                                for boc_idx in boc_atoms:
                                    ...
                                    if bond is not None:
                                        print(f"Boc-protected piperazine detected: {mol_smiles}")
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if (
                    ...
                    for reactant in reactants:
                        if checker.check_ring("piperazine", reactant):
                            print(f"Boc protection of piperazine detected in reaction: {rsmi}")
                            has_boc_piperazine = True
                            return
                ...
                if (
                    ...
                    for reactant in reactants:
                        if checker.check_ring("piperazine", reactant) and checker.check_fg(
                            "Boc", reactant
                        ):
                            print(f"Deprotection of Boc-piperazine detected in reaction: {rsmi}")
                            has_boc_piperazine = True
                            return
