        ...
        if node["type"] == "reaction":
            try:
                ...
                if is_alkylation:
                    ...
                    product_has_dibenzyl = checker.check_fg("Tertiary amine", product_smiles)

                    # Verify dibenzylation occurred
                    if amine_reactant and len(benzyl_halides) >= 1 and product_has_dibenzyl:
                        ...
                        if product_mol:
                            ...
                            if product_mol.HasSubstructMatch(benzyl_pattern):
                                matches = product_mol.GetSubstructMatches(benzyl_pattern)
                                if (
                                    len(matches) >= 2
                ...
                if not has_dibenzylation:
                    for reactant in reactants_smiles:
                        if checker.check_fg("Primary amine", reactant):
                            ...
                            product_mol = Chem.MolFromSmiles(product_smiles)
                            if product_mol and checker.check_fg("Tertiary amine", product_smiles):
                                benzyl_pattern = Chem.MolFromSmarts("c1ccccc1C[N]")
                                if product_mol.HasSubstructMatch(benzyl_pattern):
                                    matches = product_mol.GetSubstructMatches(benzyl_pattern)
                                    if len(matches) >= 2:
                                        print(f"Detected direct N-dibenzylation at depth {depth}")
                                        print(f"Reaction SMILES: {rsmi}")
                                        has_dibenzylation = True
                                        break
