...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        if node["type"] == "reaction":
            ...
            try:
                ...
                has_phenol = any(
                    r and r.HasSubstructMatch(Chem.MolFromSmarts("[OH][c]")) for r in reactants
                )
                ...
                if (has_benzyl_ester and has_carboxylic_acid) or (has_benzyl_ether and has_phenol):
                    benzyl_protection_events.append(
                        {"depth": depth, "type": "ester" if has_benzyl_ester else "ether"}
                    )
                    print(
    ...
    if len(benzyl_protection_events) >= 2:
        ...
        print(f"Found {len(benzyl_protection_events)} benzyl protection events at depths {depths}")
