        ...
        if node["type"] == "reaction" and depth <= 2:
            if "rsmi" in node.get("metadata", {}):
                ...
                if checker.check_reaction("Reduction of aldehydes and ketones to alcohols", rsmi):
                    print(f"Found late-stage aldehyde reduction reaction at depth {depth}")
                    ...
                    aldehyde_in_products = any(checker.check_fg("Aldehyde", p) for p in products)

                    if alcohol_in_reactants and aldehyde_in_products:
                        print(f"Found late-stage aldehyde reduction (by FG check) at depth {depth}")
                        late_stage_reduction_found = True
