...
def main(route):
    ...
    def has_biaryl_bond(mol):
        ...
        for i in range(len(rings)):
            ...
            for j in range(i + 1, len(rings)):
                ...
                if all(mol.GetAtomWithIdx(idx).GetIsAromatic() for idx in ring1) and all(
                    mol.GetAtomWithIdx(idx).GetIsAromatic() for idx in ring2
                ):
                    ...
                    if depth >= early_stage_threshold:
                        ...
                        for rxn_name in ring_formation_reactions:
                            if checker.check_reaction(rxn_name, rsmi):
                                ...
                                print(f"Ring formation detected at depth {depth}: {rxn_name}")
                                break
                        ...
                        if ring_formation_depth == -1:
                            ...
                            reactant_rings_count = sum(
                                len(mol.GetRingInfo().AtomRings()) for mol in reactant_mols if mol
                            )
                            product_rings_count = len(product_mol.GetRingInfo().AtomRings())
                    ...
                    if mid_stage_threshold - 1 <= depth <= mid_stage_threshold + 1:
                        ...
                        for rxn_name in biaryl_coupling_reactions:
                            if checker.check_reaction(rxn_name, rsmi):
                                ...
                                print(f"Biaryl coupling detected at depth {depth}: {rxn_name}")
                                break
                        ...
                        if biaryl_coupling_depth == -1:
                            ...
                            if has_biaryl_bond(product_mol):
                                biaryl_in_reactants = any(
                                    has_biaryl_bond(r_mol) for r_mol in reactant_mols if r_mol
                                )
                            ...
                            if biaryl_coupling_depth == -1:
                                reactant_aromatic_rings = sum(
                                    count_aromatic_rings(r_mol) for r_mol in reactant_mols if r_mol
                                )
                                product_aromatic_rings = count_aromatic_rings(product_mol)
    ...
    result = (
        ring_formation_depth >= early_stage_threshold
        and (mid_stage_threshold - 1 <= biaryl_coupling_depth <= mid_stage_threshold + 1)
        and late_fg_mod
    )

    print(f"Strategy detection result: {result}")
    print(f"Ring formation depth: {ring_formation_depth} (threshold: >={early_stage_threshold})")
    print(f"Biaryl coupling depth: {biaryl_coupling_depth} (threshold: ~{mid_stage_threshold})")
