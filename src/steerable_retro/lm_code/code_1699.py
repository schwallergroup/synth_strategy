    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "reaction":
            ...
            if product and all(reactants):
                ...
                heterocycle_patterns = [indole_pattern, pyridine_pattern, pyrrole_pattern]

                has_heterocycle = any(
                    any(mol.HasSubstructMatch(pattern) for pattern in heterocycle_patterns)
                    for mol in reactants
                )

                if has_boronic and has_halide and has_heterocycle and depth <= 1:  # Late-stage
                    found_heterocycle_suzuki = True
                    print(f"Found heterocycle-containing Suzuki coupling at depth {depth}")
