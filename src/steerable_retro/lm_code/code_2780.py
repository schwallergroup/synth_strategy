        ...
        if node["type"] == "reaction":
            if "rsmi" in node.get("metadata", {}):
                ...
                try:
                    ...
                    if product_mol:
                        product_c_n_bonds = len(product_mol.GetSubstructMatches(c_n_bond_pattern))
                    else:
                        product_c_n_bonds = 0

                    reactant_c_n_bonds = sum(
                        len(r.GetSubstructMatches(c_n_bond_pattern)) for r in reactant_mols if r
                    )
                    ...
                    if product_c_n_bonds > reactant_c_n_bonds:
                        ...
                        print(f"C-N bond formation detected, total count: {c_n_bond_formations}")
                except:
                    pass
