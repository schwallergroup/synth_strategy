...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        if node["type"] == "reaction":
            ...
            if any(
                mol.HasSubstructMatch(Chem.MolFromSmarts("[CX4][Cl]")) for mol in reactants
            ...
            if any(
                mol.HasSubstructMatch(Chem.MolFromSmarts("[CX2]#[NX1]")) for mol in reactants
            ...
            if any(
                ...
                print(f"Found carboxylic acid to alcohol transformation at depth {depth}")

            # Check for alcohol to ether transformation
            if any(
                mol.HasSubstructMatch(Chem.MolFromSmarts("[CX4][OX2H]")) for mol in reactants
