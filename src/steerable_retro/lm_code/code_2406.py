...
def main(route):
    ...
    depths_with_nitrile = set(depth for depth, has_nitrile, _ in nitrile_presence if has_nitrile)
    consecutive_depths = len(depths_with_nitrile) > 1

    # Check if nitrile is transformed in late stage (lower depth)
    late_stage_transformation = any(depth <= max_depth // 2 for depth, _ in nitrile_transformations)
    ...
    result = nitrile_in_early_stages and consecutive_depths and late_stage_transformation
