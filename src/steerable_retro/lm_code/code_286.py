...
def main(route):
    """
    This function detects synthesis routes that involve pyrazole ring formation.
    """
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "reaction":
            try:
                ...
                # Check for pyrazole in product but not in reactants
                if checker.check_ring("pyrazole", product_smiles):
                    # Check if pyrazole is formed in this step (not present in reactants)
                    ...
                    if not pyrazole_in_reactants:
                        ...
                        for r in reactants_smiles:
                            if checker.check_fg("Hydrazine", r):
                                print("Pyrazole formed using hydrazine")
                                break
