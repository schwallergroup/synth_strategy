...
def main(route):
                                        ...
                                        if nbr.GetSymbol() == "C":
                                            for nbr2 in nbr.GetNeighbors():
                                                if (
                                                    ...
                                                    protected_amines[atom.GetAtomMapNum()] = rsmi
                                                    print(
                        ...
                        if p_mol:
                            for atom in p_mol.GetAtoms():
                                if atom.GetAtomMapNum() > 0 and atom.GetSymbol() == "N":
                                    ...
                                    if atom.GetTotalNumHs() >= 1 or atom.GetFormalCharge() > 0:
                                        deprotected_amines[atom.GetAtomMapNum()] = rsmi
                                        print(f"Deprotected amine atom ID: {atom.GetAtomMapNum()}")

                # Special case for acetyl deprotection (common in the test cases)
                for reactant in reactants:
                    if checker.check_fg("Secondary amide", reactant) or checker.check_fg(
                        "Tertiary amide", reactant
                    ):
    ...
    if common_atoms:
        print(f"Found {len(common_atoms)} amine(s) that were both protected and deprotected")
        return True
    ...
    if protected_amines and deprotected_amines:
        print("Found both protection and deprotection steps, but couldn't match specific atoms")
        return True
    ...
    if not protected_amines and deprotected_amines:
        print("Found deprotection steps but no protection steps. Assuming protection occurred.")
        return True
