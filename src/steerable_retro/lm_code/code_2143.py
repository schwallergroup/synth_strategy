...
def main(route):
    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "reaction" and "metadata" in node and "rsmi" in node["metadata"]:
            ...
            if (
                product_mol
                and any(mol and mol.HasSubstructMatch(aryl_iodide_pattern) for mol in reactant_mols)
                and any(
                    mol and mol.HasSubstructMatch(terminal_alkyne_pattern) for mol in reactant_mols
                )
