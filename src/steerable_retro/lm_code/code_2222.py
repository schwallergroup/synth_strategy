...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol" and not node.get("in_stock", False):
            ...
            if checker.check_fg("Aromatic halide", node["smiles"]):
                ...
                molecules_with_bromine.append({"smiles": node["smiles"], "depth": depth})
                print(f"Found molecule with aromatic halide at depth {depth}: {node['smiles']}")
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if (
                    ...
                    print(f"Found coupling reaction that likely consumes aromatic halide: {rsmi}")
    ...
    if len(molecules_with_bromine) >= 3 and reaction_steps >= 2:
        ...
        if len(depths) >= 3:  # Ensure molecules at different synthesis stages have the group
