...
def main(route):
    ...
    def dfs_traverse(node, depth=0, path=None):
        ...
        if node["type"] == "reaction" and "metadata" in node and "rsmi" in node["metadata"]:
            ...
            if not is_amide_coupling:
                ...
                has_amine = any(checker.check_fg("Primary amine", r) for r in reactants if r)

                # Check if product contains amide
                has_amide = checker.check_fg("Primary amide", product) or checker.check_fg(
    ...
    result = amide_coupling_found and trifluoromethyl_fragment_involved and is_convergent
    print(f"Final result: {result}")
    print(f"- Amide coupling found: {amide_coupling_found}")
    print(f"- Trifluoromethyl aromatic fragment involved: {trifluoromethyl_fragment_involved}")
