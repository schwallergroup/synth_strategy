...
def main(route):
        ...
        if node["type"] == "reaction":
            if "rsmi" in node.get("metadata", {}):
                ...
                ester_in_product = (
                    ...
                    and not Chem.MolFromSmiles(product).HasSubstructMatch(alkene_ester_pattern)
                )
                ...
                if aldehyde_in_reactants and aminonitrile_in_product:
                    ...
                    print(f"Found aldehyde to aminonitrile transformation at depth {depth}")
                ...
                if aminonitrile_in_reactants and amide_in_product:
                    ...
                    print(f"Found aminonitrile to amide transformation at depth {depth}")
    ...
    if len(valid_transformations) >= 4:
        ...
        ordered_transformations = sorted(
            [(k, v) for k, v in transformations.items() if v is not None], key=lambda x: x[1]
        )
