    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "reaction":
            ...
            try:
                ...
                has_hydrazide = reactants_mol and reactants_mol.HasSubstructMatch(hydrazide_pattern)
            except:
                has_hydrazide = False

            try:
                product_mol = Chem.MolFromSmiles(products_part)
                has_triazole = product_mol and product_mol.HasSubstructMatch(triazole_pattern)
            except:
                has_triazole = False
