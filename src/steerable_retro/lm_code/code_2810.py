...
def main(route):
    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "reaction" and "metadata" in node and "rsmi" in node["metadata"]:
            ...
            if non_commercial_count > 1:
                print(f"Found convergent step with {non_commercial_count} non-commercial reactants")
                is_linear = False
    ...
    if is_linear:
        print(f"Route follows a linear synthesis strategy with {reaction_count} reactions")
