            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if not protection_found:
                    ...
                    if all(m is not None for m in reactant_mols) and product_mol is not None:
                        ...
                        tfacetyl_pattern = Chem.MolFromSmarts("[NX3]-C(=O)C([F])([F])([F])")

                        has_nh2 = any(
                            len(m.GetSubstructMatches(nh2_pattern)) > 0 for m in reactant_mols
                        )
                ...
                if not deprotection_found:
                    ...
                    if all(m is not None for m in reactant_mols) and product_mol is not None:
                        ...
                        has_tfacetyl = any(
                            len(m.GetSubstructMatches(tfacetyl_pattern)) > 0 for m in reactant_mols
                        )
