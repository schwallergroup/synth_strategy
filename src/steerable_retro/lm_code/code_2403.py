...
def main(route):
            ...
            try:
                ...
                for reactant_smiles in reactants_smiles:
                    if checker.check_ring(
                        ...
                        if checker.check_fg("Nitrile", reactant_smiles) or checker.check_fg(
    ...
    if len(molecules_with_nitrile) >= 2 and len(molecules_with_cyclobutane) >= 1:
        ...
        nitrile_only_mols = [
            m for m in molecules_with_nitrile if not checker.check_ring("cyclobutane", m)
        ]
    ...
    for mol in molecules_with_nitrile:
        if "C1(c2c(Cl)cc(Cl)cc2Cl)CC1" in mol or "C1CC1" in mol:
            for other_mol in molecules_with_nitrile:
                if "Cc1c(Cl)cc(Cl)cc1Cl" in other_mol or "c1c(Cl)cc(Cl)cc1Cl" in other_mol:
                    print(
    ...
    strategy_present = (has_ring_opening and has_nitrile_intermediate) or has_nitrile_cyclobutane
    print(f"Ring opening with nitrile intermediates strategy detected: {strategy_present}")
    print(
