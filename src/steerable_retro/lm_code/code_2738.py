    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "reaction":
            try:
                ...
                has_ketone_in_reactant = any(checker.check_fg("Ketone", r) for r in reactants)
                ...
                if (has_ketone_in_reactant and has_alcohol_in_product) or (
                    ...
                    if checker.check_reaction("Reduction of ketone to secondary alcohol", rsmi):
                        ...
                        print(f"Found general ketone/aldehyde reduction at depth {depth}: {rsmi}")
                        has_ketone_reduction = True
                    # Check for oxidation reactions (retrosynthetic direction)
                    elif checker.check_reaction("Oxidation of secondary alcohol to ketone", rsmi):
                        ...
                        print(f"Found general alcohol oxidation at depth {depth}: {rsmi}")
                        has_ketone_reduction = True
                    # Check for Grignard reactions that can form alcohols from ketones
                    elif checker.check_reaction("Grignard from ketone to alcohol", rsmi):
                        print(f"Found Grignard ketone reduction at depth {depth}: {rsmi}")
                        has_ketone_reduction = True
