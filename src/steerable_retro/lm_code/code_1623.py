    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "reaction" and depth >= 2:  # Early stage (high depth)
            if "rsmi" in node.get("metadata", {}):
                ...
                if product_mol:
                    propargyl_ether_pattern = Chem.MolFromSmarts("[c]-[O]-[CH2]-[C]#[C]-[c]")
                    if product_mol.HasSubstructMatch(propargyl_ether_pattern):
                        ...
                        propargyl_pattern = Chem.MolFromSmarts("[Br,Cl,I]-[CH2]-[C]#[C]")
                        ...
                        if has_phenol and has_propargyl:
                            found_coupling = True
                            print(f"Found early-stage propargyl ether coupling at depth {depth}")
