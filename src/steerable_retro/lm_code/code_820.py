        ...
        if node["type"] == "reaction" and "metadata" in node and "rsmi" in node["metadata"]:
            ...
            if len(reactants) >= 2:
                ...
                has_organometallic_coupling = any(
                    checker.check_reaction(rxn, rsmi) for rxn in organometallic_reactions
                )
                            ...
                            for indices_tuple in tertiary_alcohol_indices:
                                for indices in indices_tuple:
                                    ...
                                    if oxygen_idx is not None:
                                        oxygen_atom = product_mol.GetAtomWithIdx(oxygen_idx)
                                    ...
                                    queue = deque([(carbon_idx, 0)])  # (atom_idx, distance)
                                    aromatic_systems = set()

                                    while queue:
                                        current_idx, distance = queue.popleft()
                                        current_atom = product_mol.GetAtomWithIdx(current_idx)

                                        # If we're at an aromatic atom, mark its ring system
                                        if current_atom.GetIsAromatic() and distance > 0:
