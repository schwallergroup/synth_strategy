...
def main(route):
        ...
        if node["type"] == "mol":
            ...
            try:
                ...
                if not any(
                    ...
                    scaffold_construction_reactions.append(("piperazine_formation", rsmi))

                if not any(
                    checker.check_ring("diazepane", r) for r in reactants
                ) and checker.check_ring("diazepane", product):
                    scaffold_construction_reactions.append(("diazepane_formation", rsmi))

                if not any(
                    checker.check_ring("morpholine", r) for r in reactants
                ) and checker.check_ring("morpholine", product):
                    scaffold_construction_reactions.append(("morpholine_formation", rsmi))

                if not any(
                    checker.check_ring("piperidine", r) for r in reactants
                ) and checker.check_ring("piperidine", product):
                    scaffold_construction_reactions.append(("piperidine_formation", rsmi))

                # Check if the reaction connects pyrazole and piperazine/related rings
                has_pyrazole_reactants = any(checker.check_ring("pyrazole", r) for r in reactants)
                has_piperazine_reactants = any(
                    checker.check_ring("piperazine", r) for r in reactants
                )
                has_diazepane_reactants = any(checker.check_ring("diazepane", r) for r in reactants)
                ...
                if any(
                    ...
                    if any(checker.check_fg("Secondary amine", r) for r in reactants) or any(
                        checker.check_fg("Primary amine", r) for r in reactants
                    ):
                        scaffold_construction_reactions.append(("piperazine_reaction", rsmi))
    ...
    print(f"Molecules with piperazine or related rings: {len(molecules_with_piperazine)}")
    ...
    print(f"Strategy pyrazole_piperazine_scaffold_construction: {has_scaffold_strategy}")
