...
def main(route):
        ...
        if node["type"] == "reaction":
            try:
                ...
                if (
                    ...
                    ether_formed = True
                    print(f"Detected specific ether formation reaction at depth {depth}")
                ...
                if not ether_formed:
                    ...
                    if product_has_ether and not reactants_have_ether:
                        ...
                        if has_alcohol:
                            ether_formed = True
                            print(f"Detected ether formation by FG analysis at depth {depth}")
                ...
                if ether_formed:
                    ...
                    print(f"Detected ether formation at depth {depth}, ID: {current_id}")
                ...
                if (
                    ...
                    ester_formed = True
                    print(f"Detected specific ester formation reaction at depth {depth}")
                ...
                if not ester_formed:
                    ...
                    if product_has_ester and not reactants_have_ester:
                        ...
                        if (has_acid and has_alcohol) or (has_acyl_halide and has_alcohol):
                            ester_formed = True
                            print(f"Detected ester formation by FG analysis at depth {depth}")
                ...
                if ester_formed:
                    ...
                    print(f"Detected ester formation at depth {depth}, ID: {current_id}")
                ...
                if checker.check_fg("Ether", product_smiles) and checker.check_fg(
                    ...
                    if not reactants_have_both:
                        print(f"Detected product with both ether and ester at depth {depth}")
    ...
    if len(ether_ester_events) >= 1:
        ...
        for event in ether_ester_events:
            print(f"  Type: {event['type']}, Depth: {event['depth']}, ID: {event['id']}")
        ...
        if len(ether_ester_events) >= 3:
            ...
            path_components = [event["id"].split("-")[:-1] for event in ether_ester_events]

            # Check if there's a common path prefix for at least 3 formations
            for i in range(len(path_components)):
                for j in range(i + 1, len(path_components)):
                    common_prefix = []
                    for k in range(min(len(path_components[i]), len(path_components[j]))):
                    ...
                    for idx, event in enumerate(ether_ester_events):
                        ...
                        if len(event_path) >= len(common_prefix) and all(
                            event_path[k] == common_prefix[k] for k in range(len(common_prefix))
                        ):
