...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        if node["type"] == "reaction" and "metadata" in node and "rsmi" in node["metadata"]:
            ...
            if is_deprotection_reaction or (reactant_has_silyl and product_has_alcohol):
                ...
                for r in reactants:
                    if checker.check_fg("Silyl protective group", r) or checker.check_fg(
                        "TMS ether protective group", r
                    ):
    ...
    if not protection_deprotection_found:
        protection_found = any(status.get("protected") for status in protected_molecules.values())
    ...
    print(f"Final result - Protection-deprotection strategy found: {protection_deprotection_found}")
