...
def main(route):
    ...
    if len(transformations) >= 2 and linear_synthesis:
        ...
        sorted_transformations = sorted(transformations, key=lambda x: x["depth"], reverse=True)
        ...
        for i in range(len(sorted_transformations)):
            ...
            # Check for acid chloride → ester
                ...
                pattern_count += 1
                print(f"Found acid chloride → ester pattern at depth {current['depth']}")
            ...
            elif "carboxylic_acid" in current["from"] and "acid_chloride" in current["to"]:
                pattern_count += 1
            ...
            # Check for acid chloride → amide
                ...
                pattern_count += 1
                print(f"Found acid chloride → amide pattern at depth {current['depth']}")
    ...
    if strategy_detected:
        print(f"Detected linear synthesis with functional group interconversion sequence")
        for t in sorted(transformations, key=lambda x: x["depth"], reverse=True):
            print(f"  Depth {t['depth']}: {t['from']} → {t['to']} (Reaction: {t['reaction_type']})")
    else:
