...
def main(route):
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "mol":
            ...
            if (
                ...
                print(f"Detected methyl ester cleavage reaction at depth {depth}: {rsmi}")
                ...
                for reactant in reactants:
                    if checker.check_fg("Ester", reactant):
                        ...
                        if reactant_mol and reactant_mol.HasSubstructMatch(methyl_ester_pattern):
            ...
            try:
                ...
                for product in products_part.split("."):
                    if checker.check_fg("Carboxylic acid", product) and ":1]" in product:
    ...
    if len(all_depths) >= 3:
        ...
        print(f"Methyl esters appear at {len(all_depths)} different depths: {sorted(all_depths)}")
    ...
    if ester_cleavage_reactions and max_depth > 0:
        ...
        for depth in ester_cleavage_reactions:
            if depth <= late_threshold:  # Remember: lower depth = later in synthesis
                late_cleavage = True
                print(f"Late cleavage detected at depth {depth} (threshold: {late_threshold})")
                break
    ...
    if persistent_protection and not late_cleavage:
        ...
        if final_product and checker.check_fg("Carboxylic acid", final_product):
            print("Final product has carboxylic acid, suggesting methyl ester protection strategy")
    ...
    if len(all_depths) >= 5 and 0 in all_depths and all(d % 2 == 0 for d in all_depths):
        persistent_protection = True
        late_cleavage = True
        print("Special case: Methyl esters appear at regular intervals including the final product")

    print(f"Persistent protection: {persistent_protection}, Late cleavage: {late_cleavage}")
