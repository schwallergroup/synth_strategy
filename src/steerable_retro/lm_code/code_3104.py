...
def main(route):
    ...
    def dfs_traverse(node):
        ...
        if node["type"] == "mol" and not node.get("children"):  # Final product (leaf node)
            smiles = node["smiles"]
            ...
            if mol:
                ...
                if mol.HasSubstructMatch(pyridine_pattern):
                    heterocycle_count += mol.GetSubstructMatches(pyridine_pattern).__len__()
                if mol.HasSubstructMatch(piperidine_pattern):
                    heterocycle_count += mol.GetSubstructMatches(piperidine_pattern).__len__()
                if mol.HasSubstructMatch(piperazine_pattern):
                    heterocycle_count += mol.GetSubstructMatches(piperazine_pattern).__len__()
