        ...
        if node["type"] == "reaction" and not epoxide_formation_found:
            if "rsmi" in node.get("metadata", {}):
                ...
                try:
                    ...
                    reactant_has_epoxide = any(checker.check_ring("oxirane", r) for r in reactants)
                    ...
                    if reactant_has_epoxide and product_has_hydroxyl and not product_has_epoxide:
                        print(f"Epoxide ring opening detected at depth {depth}")
                        epoxide_formation_found = True
                        return

                    # Case 3: Hydroxyl to epoxide conversion
                    if (reactant_has_hydroxyl or reactant_has_halohydrin) and product_has_epoxide:
                        print(f"Hydroxyl to epoxide conversion detected at depth {depth}")
                        epoxide_formation_found = True
                        return
