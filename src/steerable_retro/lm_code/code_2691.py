        ...
        if node["type"] == "mol":
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                if len(reactant_heterocycles) >= 1 and len(product_heterocycles) >= 1:
                    ...
                    if len(reactant_heterocycles) >= 2 and reactant_heterocycles.issubset(
                        product_heterocycles
                    ):
                        has_heterocycle_coupling = True
                        print(f"Detected multiple heterocycle preservation at depth {depth}")

                    # Case 2: At least one heterocycle in reactants and multiple in product
                    elif len(product_heterocycles) >= 2 and reactant_heterocycles.intersection(
                        product_heterocycles
                    ):
                        has_heterocycle_coupling = True
                ...
                if not is_coupling and len(reactants) >= 2:
                    ...
                    if len(reactant_heterocycles) >= 1 and len(product_heterocycles) >= 2:
                        # Check if product contains all reactant heterocycles
                        if reactant_heterocycles.issubset(product_heterocycles):
                            has_heterocycle_coupling = True
                            print(f"Detected implicit heterocycle coupling at depth {depth}")
