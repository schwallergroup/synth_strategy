...
def main(route):
    ...
    if protection_events:
        ...
        for depth, reactant, product in protection_events:
            if has_other_reactive_groups(reactant) or has_other_reactive_groups(product):
                print(
    ...
    def check_intermediates(node, depth=0, has_ester=False):
        if node["type"] == "mol" and "smiles" in node:
            ...
            if checker.check_fg("Ester", mol_smiles) and has_other_reactive_groups(mol_smiles):
                print(f"Found intermediate with ester and other reactive groups at depth {depth}")
                return True
