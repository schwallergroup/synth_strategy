            ...
            if node["smiles"]:
                ...
                for ring in heterocycle_rings:
                    if checker.check_ring(ring, mol_smiles):
                        ...
                        print(f"Heterocycle {ring} detected in molecule at depth {depth}")
                        break
                ...
                if heterocycle_found:
                    for fg in functional_groups:
                        if checker.check_fg(fg, mol_smiles):
                            ...
                            print(f"Functional group {fg} detected in molecule at depth {depth}")
            ...
            if "metadata" in node and "rsmi" in node["metadata"]:
                ...
                for ring in heterocycle_rings:
                    if checker.check_ring(ring, product):
                        ...
                        print(f"Heterocycle {ring} detected in product at depth {depth}")
                        break
                ...
                for reactant in reactants:
                    for ring in heterocycle_rings:
                        if checker.check_ring(ring, reactant):
                            ...
                            print(f"Heterocycle {ring} detected in reactant at depth {depth}")
                            break
                ...
                for reaction_type in functionalization_reactions:
                    if checker.check_reaction(reaction_type, rsmi):
                        ...
                        print(f"Functional group change detected: {reaction_type} at depth {depth}")
                        break

                # If no predefined reaction detected, check for functional group changes directly
                if not reaction_detected and product_has_heterocycle and reactants_with_heterocycle:
