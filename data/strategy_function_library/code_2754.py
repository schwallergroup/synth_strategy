...
def main(route) -> Tuple[bool, Dict]:
    ...
    def dfs_traverse(node, depth=0):
        nonlocal all_nodes_have_cyclohexane, findings_json
        ...
        if node["type"] == "mol" and "smiles" in node:
            ...
            if has_cyclohexane:
                # Record the finding if cyclohexane is present
    ...
    # If all nodes had cyclohexane, add the structural constraint
    if all_nodes_have_cyclohexane:
        findings_json["structural_constraints"].append({
            "type": "negation",
            "details": {
                "target": "absence_of_cyclohexane_ring",
                "scope": "non_stock_molecules"
            }
        })

    return all_nodes_have_cyclohexane, findings_json
