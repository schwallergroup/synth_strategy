...
def main(route) -> Tuple[bool, Dict]:
    ...
    def dfs_traverse(node, depth=0):
        nonlocal amide_coupling_depth, findings_json
        ...
        if node["type"] == "reaction":
            if "rsmi" in node.get("metadata", {}):
                try:
                    ...
                    for reaction_type in AMIDE_COUPLING_REACTION_TYPES:
                        if checker.check_reaction(reaction_type, rsmi):
                            is_amide_coupling = True
                            findings_json["atomic_checks"]["named_reactions"].append(reaction_type)
                            break

                    if is_amide_coupling and amide_coupling_depth is None:
        ...
        # Traverse children
        if amide_coupling_depth is None:
    ...
    if amide_coupling_depth is not None and amide_coupling_depth <= 1:
        ...
        findings_json["structural_constraints"].append({
            ...
            "details": {
                ...
                "position": "late_stage (last two steps)"
            }
        })
