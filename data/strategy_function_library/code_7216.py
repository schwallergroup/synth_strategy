...
def main(route) -> Tuple[bool, Dict]:
    """
    Detects a late-stage amide coupling strategy by checking for a specific set of named reactions (e.g., Schotten-Baumann, acylation of amines) within the final two synthetic steps.
    """
    findings_template = {
        "atomic_checks": {
            "named_reactions": [],
    ...
    def dfs_traverse(node, depth=0):
        ...
        if amide_formation_detected:  # Optimization: stop if already found
            return
        ...
        if node["type"] == "reaction" and depth <= 2:
            try:
                ...
                if is_amide_coupling:
                    ...
                    # Add the structural constraint if an amide coupling is found within the depth
                    findings_json["structural_constraints"].append({
                        "type": "positional",
                        "details": {
                            "target": "any_amide_coupling_reaction",
                            "position": "within_final_3_steps"
                        }
                    })

            except (KeyError, IndexError):
                # Fail silently on malformed reaction data
                pass
