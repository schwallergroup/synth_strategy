...
def main(route) -> Tuple[bool, Dict]:
    """
    Detects a late-stage amide formation strategy. This is identified if the final or penultimate reaction step (depth <= 1) matches a defined set of reaction types, including 'Acylation of Nitrogen Nucleophiles by Carboxylic Acids', 'Carboxylic acid with primary amine to amide', 'Acyl chloride with primary amine to amide (Schotten-Baumann)', 'Acyl chloride with secondary amine to amide', 'Ester with primary amine to amide', and 'Ester with secondary amine to amide'.
    """
    findings_template = {
        "atomic_checks": {
            "named_reactions": [],
        ...
        if node["type"] == "reaction" and depth <= 1:  # Late stage
            if "mapped_reaction_smiles" in node.get("metadata", {}):
                try:
                    ...
                    # Check for amide formation using a defined list of reaction types
