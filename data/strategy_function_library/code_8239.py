...
def main(route) -> Tuple[bool, Dict]:
    """
    Detects a late-stage amide formation strategy. It specifically checks if the
    final synthetic step (depth=1) corresponds to a reaction in the
    LATE_STAGE_AMIDE_FORMATION_REACTIONS list.
    """
    findings_template = {
        "atomic_checks": {
            "named_reactions": [],
    ...
    def dfs_traverse(node, depth=0, path=[]):
        ...
        if node["type"] == "reaction":
            # Correctly check if this is the final reaction step (depth=1)
            if depth == 1:
