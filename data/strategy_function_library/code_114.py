...
def main(route) -> Tuple[bool, Dict]:
    ...
    by checking for a specific set of amide-forming named reactions.
    """
    findings_template = {
        "atomic_checks": {
            "named_reactions": [],
    ...
    def dfs_traverse(node, depth=0):
        ...
        if node["type"] == "reaction" and depth <= 1:
            try:
                ...
                is_amide_reaction_found = False
                for r in AMIDE_FORMATION_REACTIONS:
                    if checker.check_reaction(r, rsmi):
                        is_amide_reaction_found = True
                        findings_json["atomic_checks"]["named_reactions"].append(r)
                        break

                if is_amide_reaction_found:
