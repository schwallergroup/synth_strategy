...
def main(route) -> Tuple[bool, Dict]:
    """
    Detects if a synthesis employs a complete Boc protection strategy for amines. A complete strategy requires finding at least one reaction from a predefined list of Boc protection transformations (BOC_PROTECTION_REACTIONS) and at least one reaction from a predefined list of Boc deprotection transformations (BOC_DEPROTECTION_REACTIONS) within the route.
    """
    findings_template = {
        "atomic_checks": {
            "named_reactions": [],
    ...
    result = found_protection and found_deprotection

    if found_protection:
        findings_json["structural_constraints"].append({
            "type": "count",
            "details": {
                "target": "Boc_protection_reaction",
                "operator": ">=",
                "value": 1
            }
        })
    if found_deprotection:
        findings_json["structural_constraints"].append({
            "type": "count",
            "details": {
                "target": "Boc_deprotection_reaction",
                "operator": ">=",
                "value": 1
            }
        })

    print(f"Protection found: {found_protection}, Deprotection found: {found_deprotection}")
