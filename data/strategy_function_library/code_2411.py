...
LATE_STAGE_AROMATIC_HALOGENATION_REACTIONS = [
...
def main(route) -> Tuple[bool, Dict]:
    """
    Detects late-stage aromatic halogenation by checking for specific named reactions (Aromatic fluorination, chlorination, bromination, or iodination) in the final synthesis step.
    """
    findings_template = {
        "atomic_checks": {
            "named_reactions": [],
        ...
        if node["type"] == "reaction" and depth == 1:  # Final step (depth 1 in retrosynthesis)
            try:
                if "rsmi" in node.get("metadata", {}):
                    ...
                    for reaction_type in LATE_STAGE_AROMATIC_HALOGENATION_REACTIONS:
                        if checker.check_reaction(reaction_type, rsmi):
                            found_late_stage_halogenation = True
                            findings_json["atomic_checks"]["named_reactions"].append(reaction_type)
                            # Add the structural constraint if a late-stage halogenation is found
                            ...
                            break # Only need to find one to set the flag and add the constraint
            except Exception:
                # Silently ignore errors in single reaction analysis
                pass
